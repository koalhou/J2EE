...
Class.pt.move = function(){
	...
	$(config.move[0]).bind('mousedown', function(M){	
		M.preventDefault();
...
Class.pt.callback = function(){
	...
	ready.iE6 && this.IE6();
	layerE.find('.xubox_close').unbind('click').bind('click', function(e){
		e.preventDefault();
		config.close(that.index);
	});
	layerE.find('.xubox_yes').unbind('click').bind('click',function(e){
		e.preventDefault();
		dialog.yes(that.index);
	});
	layerE.find('.xubox_no').unbind('click').bind('click',function(e){
		e.preventDefault();
		dialog.no(that.index);
	});
	this.layerS.unbind('click').bind('click', function(e){
		e.preventDefault();
