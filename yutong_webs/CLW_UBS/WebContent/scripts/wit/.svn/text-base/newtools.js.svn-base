...
Wit.removeAllSpan = function(elm) {
	...
	for(var i = 0, len = pnode.length; i < len; i++){
		if(pnode[i].getTag() == 'span' || pnode[i].getTag() == 'div'){
			pnode[i].remove();
		}
	}
};
...
function bind(o,e,f){
    /MSIE/.test(navigator.userAgent)? o.attachEvent("on"+e,function(){f.apply(o,arguments);}):o.addEventListener(e,f,false);
  };
  
Wit.showErr = function(elm, msg, styleName) {
	Wit.removeAllSpan(elm);
	var id = elm.id + '_div';
	var spanElement = new Element('div', {
		'id'    : id,
		'class' : 'errorTips',
		'style' : 'display:none'
	});
	...
	// 填加边框
	spanElement.style.border = "2px solid #DDD";
	spanElement.style.background = "#C00";
	spanElement.style.color = "white";
	spanElement.style.padding = "4px 10px 4px 10px";
	spanElement.style.filter = "progid:DXImageTransform.Microsoft.Shadow(color=#909090,direction=120,strength=4)";
	**/
	spanElement.injectAfter(elm);
	var sn = styleName == undefined ? 'tf' : styleName;
	elm.className = sn + ' ' + 'errorInput';
	
	bind(elm,"mousemove",function(e){
		if(document.getElementById(id) != null) {
			document.getElementById(id).style.display = "block";
		}
	});
	bind(elm,"mouseout",function(e){
		if(document.getElementById(id) != null) {
