	...
	$.Poshytip.prototype = {
		...
		refresh: function(async) {
			...
			if (arrow && this.pos.arrow) {
				this.$arrow[0].className = 'tip-arrow tip-arrow-' + this.pos.arrow;
				//zyong add，修改tip如果在左右靠边的时候，箭头无法指向鼠标事件的位置。
				if(this.pos.arrow=="top"||this.pos.arrow=="bottom")
				{
					var w=$(window).width()-this.eventX;
					if(w<this.$tip.width())
					{
						
						var t1= Math.abs(this.pos.l-this.eventX);
						
						if(t1>(this.$tip.width()-40))
							t1=this.$tip.width()-40;
						$(this.$arrow[0]).css({left:t1});
					}
					else
					{
						$(this.$arrow[0]).css({left:0});
					}
				}
				//zyong add
