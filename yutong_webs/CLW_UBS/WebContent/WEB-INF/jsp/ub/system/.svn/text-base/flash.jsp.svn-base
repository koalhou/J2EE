...
function searchChartData()
{
	...
	var url='<s:url value="/ubs/flashList.shtml" />'+'?searchparam.startDate='+startDate+'&searchparam.endDate='+endDate;
	jQuery.ajax({
		url : url,
		cache : false,
		dataType : "json",
...
function loadChart(data)
{
	...
	if(indicators.length>1)
	{
		...
		strbarms.push("<chart caption='' xAxisName='Flash版本' yAxisName='次数' >");
	...
	{
		var sbar=[];
		sbar.push("<chart caption='Flash版本'  xAxisName='Flash版本' rotateYAxisName='0' yAxisName='次数' >");
		var offset=0;
		var indcttext="";
...
jQuery(function() {
	...
	roleList.flexigrid({
		...
		url: '<s:url value="/ubs/flashPageList.shtml" />',
		dataType: 'json',
		height: 'auto',
		width: 'auto',
		colModel : [
		    		{display: '<s:text name="ub.system.flash" />', name : 'systemname', width : '200', sortable : true, align: 'left'},
