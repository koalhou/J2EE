...
function searchChartData()
{
	...
	var url='<s:url value="/ubs/solutionList.shtml" />'+'?searchparam.startDate='+startDate+'&searchparam.endDate='+endDate;
	jQuery.ajax({
		url : url,
		cache : false,
		dataType : "json",
...
function loadChart(data)
{
	...
	if(indicators.length>1)
	{
		...
		strbarms.push("<chart caption='' xAxisName='分辨率' yAxisName='次数' >");
	...
	{
		var sbar=[];
		sbar.push("<chart caption='屏幕分辨率'  xAxisName='分辨率' rotateYAxisName='0' yAxisName='次数' >");
		var offset=0;
		var indcttext="";
...
jQuery(function() {
	...
	roleList.flexigrid({
		...
		url: '<s:url value="/ubs/solutionPageList.shtml" />',
		dataType: 'json',
		height: 'auto',
		width: 'auto',
		colModel : [
		    		{display: '<s:text name="ub.system.solution" />', name : 'systemname', width : '200', sortable : true, align: 'left'},
