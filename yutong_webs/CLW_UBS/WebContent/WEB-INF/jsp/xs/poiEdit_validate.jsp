...
<script type="text/javascript">
  ...
  function submitForm() {
    ...
    var f10 = checkEamil();
    var f11 = checkPoiService();
    var f12 = checkMessageInput();
    var f13 = checkConcateTel();
    var f14 = checkConcatePerson();
	if (f1 && f2 && f3 && f4 && f5 && f6 && f7 && f10 && f11 && f12 && f13 && f14) {
      var oldPoiCode = $('poiCodeOld');
	  var newPoiCode = $('poiCode');
		  
	  if(oldPoiCode.value != newPoiCode.value) {
        var f11 = checkPoiCodeUnique();
        if(f11) {
          if(confirm("<s:property value="getText('confirm.modify')" />")) {
		    Wit.commitAll($('poiedit_form'));
          } 
        }
	  } else {
		// 网点代码无变化时，不检查唯一性
  ...
  /** 有效设置 **/
  function poiStartUse(url) {
    if(confirm("<s:property value="getText('poi.startuse.confirm')" />")) {
      Wit.goBack(url);
    } else {
      return false;
    }
  ...
  /** 无效设置 **/
  function poiStopUse(url) {
    if(confirm("<s:property value="getText('poi.stopuse.confirm')" />")) {
      Wit.goBack(url);
    } else {
      return false;
    }
