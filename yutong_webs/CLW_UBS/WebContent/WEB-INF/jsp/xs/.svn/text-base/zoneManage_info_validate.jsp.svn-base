...
function submitPostEditFrom() {
	...
	if(f1&&f2&&f3&&f4){
		if(confirm("<s:property value="getText('confirm.modify')" />")) {
			var fm=$('clwForm');
			fm.action="<s:url value='/xs/zonemanage_do_edit.shtml' />";
			Wit.commitAll(fm);
	     }
	}
}
function submitPostDelFrom(){
	...
	if(confirm("<s:property value="getText('confirm.delete')" />")) {
		var fm=$('clwForm');
		fm.action="<s:url value='/xs/zonemanage_do_del.shtml' />";
		Wit.commitAll(fm);
	}else {
		return;
	}
...
function checkZONE_NAME_only(){
	var elm = $('zone_name_input');
	var elm_old = $('zone_name_input_old');
