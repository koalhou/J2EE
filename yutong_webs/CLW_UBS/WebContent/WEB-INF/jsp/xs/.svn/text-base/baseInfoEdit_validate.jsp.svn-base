...
<script type="text/javascript">
  ...
  function submitForm() {
	...
	if (f1 && f2 && f3 && f4 && f6) {
      var codeIdOld = $('codeIdOld');
      var codeId = $('codeId');

      var codeTypeOld = $('codeTypeOld');
      var codeType = $('codeType');
	      
      if(codeIdOld.value != codeId.value || codeTypeOld.value != codeType.value) {
        // CODEID变化时，检查CODEID唯一性  
        var f5 = checkCodeIdUnique();
        if(f5) {
          if(confirm("<s:property value="getText('confirm.modify')" />")) {
            Wit.commitAll($('baseinfoedit_form'));
          } 
        }
      } else {
        // CODEID无变化时，不检查CODEID唯一性
        if(confirm("<s:property value="getText('confirm.modify')" />")) {
          Wit.commitAll($('baseinfoedit_form'));
        } 
      }
