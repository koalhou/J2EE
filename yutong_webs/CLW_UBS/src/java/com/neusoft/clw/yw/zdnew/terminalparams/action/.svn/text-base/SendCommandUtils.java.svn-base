package com.neusoft.clw.yw.zdnew.terminalparams.action;
...
import com.neusoft.clw.yw.zdnew.terminalparams.ds.Bit32ValueInfo;
...
public class SendCommandUtils {
    ...
    private String analyseTerminalParams(XCTerminalParamsInfo bean) {
        ...
        if(checkValue(bean.getStandbyIp())) {
            sb.append(getASCIIPacketContent("17", bean.getStandbyIp()));
        }
        // 18-服务器TCP端口
        ...
        if(checkValue(bean.getDriverOverDateTime())) {
            sb.append(getParamHexPacketContent("22", bean.getDriverOverDateTime()));
        }
        // 27-休眠时汇报时间间隔
        ...
        if(checkValue(bean.getListenPhone())) {
            sb.append(getASCIIPacketContent("48", bean.getListenPhone()));
        }
        // 50-报警屏蔽字
        if(checkCheckBoxStatus(bean.isAlarmShieldFlag())) {
            sb.append(getBinary0ToHex("50", bean.getAlarmShield(), 8));
        }
        // 52-报警拍摄开关
        if(checkCheckBoxStatus(bean.isAlarmShootFlag())) {
            sb.append(getBinary1ToHex("52", bean.getAlarmShootSwitch(), 8));
        }
        // 53-报警拍摄存储标志
        if(checkCheckBoxStatus(bean.isAlarmShootSave())) {
            sb.append(getBinary1ToHex("53", bean.getAlarmShootSaveFlag(), 8));
        }
        // 55-最高速度
        if(checkValue(bean.getTopSpeed())) {
            sb.append(getParamHexPacketContent("55", bean.getTopSpeed()));
        }
        // 56-超速持续时间
        if(checkValue(bean.getOverspeedTime())) {
            sb.append(getParamHexPacketContent("56", bean.getOverspeedTime()));
        }
        // 5b-超速报警预警差值
        if(checkValue(bean.getOverspeedAlarmDifference())) {
            sb.append(getParamHex4PacketContent("5b", bean.getOverspeedAlarmDifference()));
        }
        // 5d-特征系数
        if(checkValue(bean.getCharacteristicOefficient())) {
            sb.append(getParamHex4PacketContent("5d", bean.getCharacteristicOefficient()));
        }
        // 5e-车轮每转脉冲数
        if(checkValue(bean.getWheelPulseCount())) {
            sb.append(getParamHex4PacketContent("5e", bean.getWheelPulseCount()));
        }
        // 5f-油箱容量
        if(checkValue(bean.getFuelCapacity())) {
            sb.append(getParamHex4PacketContent("5f", bean.getFuelCapacity()));
        }
        // 61-门开关拍照控制
        if(checkCheckBoxStatus(bean.isCarDoorFlag())) {
            sb.append(getBinary1ToHex("61", bean.getCarDoorControl(), 8));
        }
        // 62-终端外设安装配置
        if(checkCheckBoxStatus(bean.isTerminalOuterDeviceFlag())) {
            sb.append(getBinary1ToHex("62", bean.getTerminalOuterDevice(), 8));
        }
        // 64-定时拍照控制
        if(checkCheckBoxStatus(bean.isRegularCameraFlag())) {
            sb.append(formatScheduleData("64", bean.getRegularCameraControl(), bean.getRegularTime()));
        }
        // 65-定距拍照控制
        if(checkCheckBoxStatus(bean.isFixDistanceFlag())) {
            sb.append(formatScheduleData("65", bean.getFixDistanceCameraControl(), bean.getFixDistance()));
        }
        // 66-速度来源设置
        if(checkValue(bean.getSpeedSourceSetting())) {
            sb.append(getParamHex2PacketContent("66", bean.getSpeedSourceSetting()));
        }
        // 80-车辆里程表读数 
        ...
        if(checkValue(bean.getVehicleLn())) {
            sb.append(getASCIIPacketContent("83", bean.getVehicleLn()));
        }
        // 84-车牌颜色
        if(checkValue(bean.getVehicleLnColor())) {
            sb.append(getParamHex2PacketContent("84", bean.getVehicleLnColor()));
        }
        // f0-语音输出通道控制
        if(checkValue(bean.getVoiceOutputControlType0())) {
            sb.append(getVoiceOutputDataContent("f0", 
                                                String.format(
                                                        "%s%s%s%s%s%s%s%s", 
                                                        "0", 
                                                        "0", 
        ...
        if(checkValue(bean.getAutoSwitchTrip())) {
            sb.append(getParamHex2PacketContent("f1", bean.getAutoSwitchTrip()));
        }
        return sb.toString();
    }
