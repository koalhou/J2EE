...
public class SendCommandUtils {
    ...
    private String getParamHexPacketContent(String paramid, String setValue) {
        ...
        sb.append(paramid);
        sb.append("08");
        sb.append(format(Long.toHexString(Long.parseLong(setValue)), 
                         8));
        return sb.toString();
    }
    ...
    private String analyseTerminalParams(XCTerminalParamsInfo bean) {
        ...
        if(checkValue(bean.getKeepAliveTime())) {
            sb.append(getParamHexPacketContent(format("01",8), bean.getKeepAliveTime()));
        }
        // 02-TCP消息应答超时时间
        if(checkValue(bean.getTcpOverTime())) {
            sb.append(getParamHexPacketContent(format("02",8), bean.getTcpOverTime()));
        }
        // 03-TCP消息重传次数
        if(checkValue(bean.getTcpRetransTime())) {
            sb.append(getParamHexPacketContent(format("03",8), bean.getTcpRetransTime()));
        }
        // 04-UDP消息应答超时时间
        if(checkValue(bean.getUdpOverTime())) {
            sb.append(getParamHexPacketContent(format("04",8), bean.getUdpOverTime()));
        }
        // 05-UDP消息重传次数
        if(checkValue(bean.getUdpRetransTime())) {
            sb.append(getParamHexPacketContent(format("05",8), bean.getUdpRetransTime()));
        }
        // 10-主服务器APN
        if(checkValue(bean.getMainApn())) {
            sb.append(getASCIIPacketContent(format("10",8), bean.getMainApn()));
        }
        // 11-主服务器无线通讯拨号用户名
        if(checkValue(bean.getMainUser())) {
            sb.append(getASCIIPacketContent(format("11",8), bean.getMainUser()));
        }
        // 12-主服务器无线通讯拨号密码
        if(checkValue(bean.getMainPass())) {
            sb.append(getASCIIPacketContent(format("12",8), bean.getMainPass()));
        }
        // 13-主服务器地址
        if(checkValue(bean.getMainIp())) {
            sb.append(getASCIIPacketContent(format("13",8), bean.getMainIp()));
        }
        // 14-备份服务器APN
        if(checkValue(bean.getStandbyApn())) {
            sb.append(getASCIIPacketContent(format("14",8), bean.getStandbyApn()));
        }
        // 15-备份服务器无线通讯拨号用户名
        if(checkValue(bean.getStandbyUser())) {
            sb.append(getASCIIPacketContent(format("15",8), bean.getStandbyUser()));
        }
        // 16-备份服务器无线通讯拨号密码
        if(checkValue(bean.getStandbyPass())) {
            sb.append(getASCIIPacketContent(format("16",8), bean.getStandbyPass()));
        }
        // 17-备份服务器地址
        if(checkValue(bean.getStandbyIp())) {
            sb.append(getASCIIPacketContent(format("17",8), bean.getStandbyIp()));
        }
        // 18-服务器TCP端口
        if(checkValue(bean.getTcpPort())) {
            sb.append(getParamHexPacketContent(format("18",8), bean.getTcpPort()));
        }
        // 19-服务器UDP端口
        if(checkValue(bean.getUdpPort())) {
            sb.append(getParamHexPacketContent(format("19",8), bean.getUdpPort()));
        }
        // 20-位置汇报策略
        if(checkValue(bean.getPositionUpType())) {
            sb.append(getParamHexPacketContent(format("20",8), bean.getPositionUpType()));
        }
        // 21-位置汇报方案
        if(checkValue(bean.getPositionUpSchema())) {
            sb.append(getParamHexPacketContent(format("21",8), bean.getPositionUpSchema()));
        }
        // 22-驾驶员未登录汇报时间间隔
        if(checkValue(bean.getDriverOverDateTime())) {
            sb.append(getParamHexPacketContent(format("22",8), bean.getDriverOverDateTime()));
        }
        // 27-休眠时汇报时间间隔
        if(checkValue(bean.getSleepDateTime())) {
            sb.append(getParamHexPacketContent(format("27",8), bean.getSleepDateTime()));
        }
        // 28-紧急报警时汇报时间间隔
        if(checkValue(bean.getSosTime())) {
            sb.append(getParamHexPacketContent(format("28",8), bean.getSosTime()));
        }
        // 29-缺省时间汇报间隔
        if(checkValue(bean.getDefaultDateTime())) {
            sb.append(getParamHexPacketContent(format("29",8), bean.getDefaultDateTime()));
        }
        // 2c-缺省距离汇报间隔
        if(checkValue(bean.getDefaultSpaceTime())) {
            sb.append(getParamHexPacketContent(format("2c",8), bean.getDefaultSpaceTime()));
        }
        // 2d-驾驶员未登录汇报距离间隔
        if(checkValue(bean.getDriverOverSpaceTime())) {
            sb.append(getParamHexPacketContent(format("2d",8), bean.getDriverOverSpaceTime()));
        } 
        // 2e-休眠时汇报距离间隔
        if(checkValue(bean.getSleepSpaceTime())) {
            sb.append(getParamHexPacketContent(format("2e",8), bean.getSleepSpaceTime()));
        }
        // 2f-紧急报警时汇报距离间隔
        if(checkValue(bean.getSosSpaceTime())) {
            sb.append(getParamHexPacketContent(format("2f",8), bean.getSosSpaceTime()));
        }
        // 30-拐点补传角度
        if(checkValue(bean.getMakeUpAngle())) {
            sb.append(getParamHexPacketContent(format("30",8), bean.getMakeUpAngle()));
        }
        // 48-监听电话号码
        if(checkValue(bean.getListenPhone())) {
            sb.append(getASCIIPacketContent(format("48",8), bean.getListenPhone()));
        }
        // 50-报警屏蔽字
        if(checkCheckBoxStatus(bean.isAlarmShieldFlag())) {
            sb.append(getBinary0ToHex(format("50",8), bean.getAlarmShield(), 8));
        }
        // 52-报警拍摄开关
        if(checkCheckBoxStatus(bean.isAlarmShootFlag())) {
            sb.append(getBinary1ToHex(format("52",8), bean.getAlarmShootSwitch(), 8));
        }
        // 53-报警拍摄存储标志
        if(checkCheckBoxStatus(bean.isAlarmShootSave())) {
            sb.append(getBinary1ToHex(format("53",8), bean.getAlarmShootSaveFlag(), 8));
        }
        // 55-最高速度
        if(checkValue(bean.getTopSpeed())) {
            sb.append(getParamHexPacketContent(format("55",8), bean.getTopSpeed()));
        }
        // 56-超速持续时间
        if(checkValue(bean.getOverspeedTime())) {
            sb.append(getParamHexPacketContent(format("56",8), bean.getOverspeedTime()));
        }
        // 5b-超速报警预警差值
        if(checkValue(bean.getOverspeedAlarmDifference())) {
            String overSpeedAlarmDifference = bean.getOverspeedAlarmDifference();
        	int over = Integer.parseInt(overSpeedAlarmDifference)*10;
            sb.append(getParamHex4PacketContent(format("5b",8), String.valueOf(over).toString()));
            
        }
        // 5d-特征系数
        if(checkValue(bean.getCharacteristicOefficient())) {
            sb.append(getParamHex4PacketContent(format("5d",8), bean.getCharacteristicOefficient()));
        }
        // 5e-车轮每转脉冲数
        if(checkValue(bean.getWheelPulseCount())) {
            sb.append(getParamHex4PacketContent(format("5e",8), bean.getWheelPulseCount()));
        }
        // 5f-油箱容量
        if(checkValue(bean.getFuelCapacity())) {
            sb.append(getParamHex4PacketContent(format("5f",8), bean.getFuelCapacity()));
        }
        // 61-门开关拍照控制
        if(checkCheckBoxStatus(bean.isCarDoorFlag())) {
            sb.append(getBinary1ToHex(format("61",8), bean.getCarDoorControl(), 8));
        }
        // 62-终端外设安装配置
        if(checkCheckBoxStatus(bean.isTerminalOuterDeviceFlag())) {
            sb.append(getBinary1ToHex(format("62",8), bean.getTerminalOuterDevice(), 8));
        }
        // 64-定时拍照控制
        if(checkCheckBoxStatus(bean.isRegularCameraFlag())) {
        	int regularTime  = Integer.parseInt(bean.getRegularTime())*60;
            sb.append(formatScheduleData(format("64",8), bean.getRegularCameraControl(), String.valueOf(regularTime)));
        }
        // 65-定距拍照控制
        if(checkCheckBoxStatus(bean.isFixDistanceFlag())) {
        	 int fixDistance  = Integer.parseInt(bean.getFixDistance())*1000;
            sb.append(formatScheduleData(format("65",8), bean.getFixDistanceCameraControl(), String.valueOf(fixDistance)));
        }
        // 66-速度来源设置
        if(checkValue(bean.getSpeedSourceSetting())) {
            sb.append(getParamHex2PacketContent(format("66",8), bean.getSpeedSourceSetting()));
        }
        // 80-车辆里程表读数 
        if(checkValue(bean.getOdometer())) {
            sb.append(getParamHexPacketContent(format("80",8), bean.getOdometer()));
        }
        // 83-车牌号
        if(checkValue(bean.getVehicleLn())) {
            sb.append(getASCIIPacketContent(format("83",8), bean.getVehicleLn()));
        }
        // 84-车牌颜色
        if(checkValue(bean.getVehicleLnColor())) {
            sb.append(getParamHex2PacketContent(format("84",8), bean.getVehicleLnColor()));
        }
        // f0-语音输出通道控制
        if(checkValue(bean.getVoiceOutputControlType0())) {
            sb.append(getVoiceOutputDataContent(format("f0",8), 
                                                String.format(
                                                        "%s%s%s%s%s%s%s%s", 
                                                        "0", 
                                                        "0", 
        ...
        if(checkValue(bean.getAutoSwitchTrip())) {
            sb.append(getParamHex2PacketContent(format("f1",8), bean.getAutoSwitchTrip()));
        }
        return sb.toString();
    }
