...
//      });
  ...
  function __show(target) {
    ...
    if(typeof showFlag!='undefined' && showFlag) {
    	jQuery("#expandD").show();
    }
  }

  function __hide(target) {
	if (typeof expand != 'undefined' && typeof expandFlag != 'undefined') {
		if (expandFlag && typeof expand == 'function') {
			expand();
			return;
		}
	}
  ...
  function __set_title(target, title) {
    var opts = $.data(target, 'mk-sidelayer').options;
    $(target).find('.mk-sidelayer-bar-title').first().html(title);
  }
