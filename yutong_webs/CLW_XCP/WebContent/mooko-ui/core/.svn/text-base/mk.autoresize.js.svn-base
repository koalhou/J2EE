 ...
 *  |- AutoResize
 *  |- 参数： 
 *    |- width_exclude: 计算宽度时排除的元素，从主元素的宽度逐个减去此属性中的元素
 *    |- width_include: 计算宽度时包含的元素，该属性中所有元素的宽度应该是相同的
 *    |- width_offset:  计算宽度时偏差值
 *    |- min_width: 最小宽度
 *    |- height_exclude: 计算高度时排除的元素，从主元素的高度逐个减去此属性中的元素
 *    |- height_include: 计算高度时包含的元素，该属性中所有元素的高度应该是相同的
 *    |- height_offset：计算高度时偏差值
 *    |- min_height: 最小高度
 *  |- 依赖： 
 *    |- Parser: mk.parser.js
 *  |- 2012-08-06
