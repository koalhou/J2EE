...
function reWriteLink(tdDiv,pid,row){
   ...
   var  pageSize =jQuery('#gala').flex_rp();
    ...
    return '<a href="editOilSetbefore.shtml?oilset.carBrand='+base64encode(utf16to8(row.cell[2]))
    +'&oilset.vehicle_ln='+base64encode(utf16to8(row.cell[0]))
    +'&oilset.carType='+base64encode(utf16to8(row.cell[11]))
    +'&oilset.carTypeDesc='+base64encode(utf16to8(row.cell[3]))
    +'&oilset.check_time_code='+row.cell[4]
     +'&oilset.vehicle_vin='+row.cell[1]
    +'&oilset.check_value='+row.cell[5]
    +'&oilset.check_id='+row.cell[12]
    +'&page='+page
    +'&oilset.velTypeId='+row.cell[10]
...
jQuery(function() {
	...
	gala.flexigrid({
		...
		width:0,
		colModel : [
					{display: '车辆号', name : 'vehicle_ln', width : 120, sortable : true, align: 'center', process: reWriteLink},
                    {display: '车辆ＶＩＮ', name : 'vehicle_vin', width : 120, sortable : true, align: 'center'},
