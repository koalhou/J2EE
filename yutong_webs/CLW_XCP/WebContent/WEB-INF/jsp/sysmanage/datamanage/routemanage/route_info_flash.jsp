...
function get_page_width() {
  ...
  function initMapList() {
		...
		var control_station=jQuery('input:radio[name="routeCheck"]:checked').val();
		var addstationids="";
		var delstationids="";
				...
				for(var i=0;i<data.length;i++){
					if(data[i].LONGITUDE >=0 && data[i].LONGITUDE <=180 && data[i].LATITUDE>=0 && data[i].LATITUDE <=90){
						...
						if(data[i].CONTROL_STATION=='0'){
						    markerOption.tipOption = addDataToTipContent("",data[i].CONTROL_STATION,"↑ ",data[i].STATION_ADDR); //yg 2012-11-29
  ...
  */
  function initMapEditList(sid) {
	...
	function clickMap(event){
		...
		var k=0;
		var val=jQuery('input:radio[name="routeCheck"]:checked').val();
		//判断选择是上行线路还是下行线路
		if(val=='0'){
	 ...
	 */
	function addDataToTipContent(content,controlStation,initDirection,initLaber){
		var tipOption = new MTipOptions();
	    //将业务数据保存在对应点的MTipOptions的content中
		//说明：各字段以"英文逗号"分隔,第一位预留
	    tipOption.tipType = MConstants.HTML_CUSTOM_TIP;
	    tipOption.content = content+","+controlStation+","+initDirection+","+initLaber;
	    return tipOption;
	}
	...
	function checkRouteName() {
		...
		if (!Mat
				.checkLength(
						route_name,
						32,
						'<s:text name="routeinfo.route_name"/>' + '<s:text name="val.feild.overlength"/>' + '32'))
			return false;
		return true;
	}
