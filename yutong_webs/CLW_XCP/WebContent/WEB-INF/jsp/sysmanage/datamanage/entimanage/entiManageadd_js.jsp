...
function changeProvinceCity() {
	// 通过所属企业联动省市信息
	var enProvinceId = '<s:property value="enterpriseDataInfo.enterprise_province"/>';
	enCityId = '<s:property value="enterpriseDataInfo.enterprise_city"/>';
	jQuery('#ENTERPRISE_PROVINCE').val(enProvinceId);
	show_enterprise_city();
}

function clickEnterEvent(obj) {
	var elm=$('PARENT_ID');
	document.getElementById('PARENT_NAME').innerText = obj.attributes.getNamedItem('short_name').value;
	elm.value = obj.id;
	/*
...
function checkPARENT_ID(){
	...
	if(onfocusEnterID==null){
		...
		Wit.showErr(elm, "宇通杯企业下不可以增加新企业");
		return false;
	}else{
		Mat.showSucc(elm);
		return true;
...
function checkFULL_NAME(){
	...
	var _pareId = $('PARENT_ID');

	var _entId = null;
	
	var _eFullNameSigle = getEnterpriseFullNameSingleRes(elm.value,_entId,_pareId.value);
	if(!Mat.checkRequired(elm)){
	    Wit.showErr(elm, "<s:property value="getText('msg.check.required')" />");
	    return false;
...
function checkPOSTCODE(){
	...
	if(postcode.value.length>0){
		if(!Mat.checkDigital(postcode)||postcode.value.length!=6){
			  Wit.showErr(postcode, "<s:property value="getText('enterprise.postcode.onlynumber6')" />");	
			  return false;
		}
	}
...
function callBackFun_show_enterprise_city(data) {
	...
	if (enCityId != ''){
    	jQuery('#ENTERPRISE_CITY').val(enCityId);
    }
}
...
function submitPostFrom() {
	...
	if(f1&&f3&&f4&&f6&&f7&&f8&&f9&&f10&&f11&&f12){
...
function newload(){
	onfocusEnterID='<s:property value="enterpriseDataInfo.enterprise_id"/>';
	//var code = document.getElementById('ENTERPRISE_CODE');
	//var msgnum=  document.getElementById('MSG_NUM');
