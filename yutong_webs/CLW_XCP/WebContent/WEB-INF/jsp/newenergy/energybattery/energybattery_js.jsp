...
var pomSel1 = null;
jQuery(function() {
	pomSel1 = new newSelect(jQuery("#routeName"),120);
	pomSel1.addSelect();
	getRouteSel();
		 ...
		 success: function(data) {
			 if(data.message == "success"){
	 			...
	 			for(var i = 0,len = routeList.length; i < len; i++){
	 				...
	 				pomSel1.addOption(objs.route_id,objs.route_name);
	 			}
	 			pomSel1.seBindOption(function(){searchMotorList();});
			 } else {
...
// 			vehicleln = node.name;
// 		}
		if(confirm("确定要导出CMS系统管理信息吗？")) {
			jQuery("#organization_id").val(finalENT);
			jQuery("#vehicleVin").val(finalVIN);
			document.getElementById('vehiclestatus_form').action="<s:url value='/energyBattery/exportBatteryList.shtml' />";
