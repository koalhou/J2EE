...
var pomSel5 = null;
jQuery(function() {
	pomSel5 = new newSelect(jQuery("#routeName"),120);
	pomSel5.addSelect();
	getRouteSel();
		 ...
		 success: function(data) {
			 if(data.message == "success"){
	 			...
	 			for(var i = 0,len = routeList.length; i < len; i++){
	 				...
	 				pomSel5.addOption(objs.route_id,objs.route_name);
	 			}
	 			pomSel5.seBindOption(function(){searchMotorList();});
			 } else {
...
// 			vehicleln = node.name;
// 		}
		if(confirm("确定要导出BMS和CMS绝缘电阻信息吗？")) {
			jQuery("#organization_id").val(finalENT);
			jQuery("#vehicleVin").val(finalVIN);
			document.getElementById('vehiclestatus_form').action="<s:url value='/energyMotor/exportMontorBMS.shtml' />";
