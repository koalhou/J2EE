	...
	var galaList = jQuery('#galaList');
	 galaList.flexigrid({
		  ...
		  colModel : [ 
				...
				{display: '设置地址（十进制）', name : 'address', width : 110, sortable : true, align: 'center',escape: true},
				{display: '软件版本号', name : 'soft_Version', width : 140, sortable : true, align: 'center',escape: true},
				{display: '软件定型日期', name : 'soft_date', width : 130, sortable : true, align: 'center',escape: true},							  
		        {display: '硬件版本号', name : 'hardware_version', width : 110, sortable : true, align: 'center',escape: true},							  
	...
	jQuery.ajax({
		 ...
		 success: function(data) {
			 if(data.message == "success"){
	 			...
	 			pomSel.seBindOption(function(){searchMotorList();});
			 } else {
...
function searchMotorList() {
	...
	jQuery('#galaList').flexOptions({
		newp: 1,
		url: '<s:url value="/energyVersion/findVersionList.shtml" />',
...
// 			vehicleln = node.name;
// 		}
		if(confirm("确定要导出硬软件版本信息吗？")) {
			jQuery("#organization_id").val(finalENT);
			jQuery("#vehicleVin").val(finalVIN);
			document.getElementById('vehiclestatus_form').action="<s:url value='/energyVersion/exportVersionList.shtml' />";
