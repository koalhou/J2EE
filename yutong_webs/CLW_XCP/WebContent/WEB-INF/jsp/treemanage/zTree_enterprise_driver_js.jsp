...
<script>
	...
	function getAsyncUrl() {
	    return jQuery("#driverName").val().replace(/(^\s*)|(\s*$)/g, "")!="" ? 
	    	    '<s:url value="/treemanage/searchTreeNodesWithDriver.shtml" />?name='+
	    	    		encodeURIComponent(encodeURIComponent(jQuery("#driverName").val().replace(/(^\s*)|(\s*$)/g, ""))) : 
			    	'<s:url value="/treemanage/searchTreeNodesWithDriver.shtml" />';
	};
	...
	function onCheck(event, treeId, treeNode) {
		...
		jQuery('#choosedriverid').val(ids.substr(0,ids.length-1));
		var tab =jQuery('#changeflag').val();
		if(tab ==1)
			searchDurationList();	
		else
			searchRecordList();
