			...
			if(mapObj.getOverlayById(stationID)!=null){//地图已经存在该点,选择复选框
				...
				jQuery.ajax({
					...
					success: function(lonlatString) {
						...
						markerOption.tipOption=addDataToTipContent("","1","listAdd",checkValue[2],checkValue[3],checkValue[2],checkValue[3],checkValue[4],checkValue[5],checkValue[6],checkValue[7]);					
						var Marker="";
...
function clickMap(event){
	...
	if(publicMapID!=event.overlayId){//（if1-start）如果触发事件的ID不是前一次缓存的ID (start)
		if(publicMapID!="BE66855BD543127FE0440019BB600AC6"){//(if2-start)
			...
			jQuery("#stationIDcontent").children("#dataTable").children("tbody").children("tr").eq(1).children("td").eq(3).children("#site_longitude").val(getPointSAByObj(mk,"realLng"));
			jQuery("#stationIDcontent").children("#dataTable").children("tbody").children("tr").eq(2).children("td").eq(3).children("#site_latitude").val(getPointSAByObj(mk,"realLat"));
			//坐标点站点业务处理判断开始
	...
	if(getPointSAByObj(mk,"pointType")!="0"){//站点业务处理-start
		...
		if(mk.option.labelOption.content!=''){
			...
			jQuery("#stationIDcontent").children("#dataTable").children("tbody").children("tr").eq(2).children("td").eq(1).children("#control_station").val(getPointSAByObj(mk,"control_station"));
			jQuery("#stationIDcontent").children("#dataTable").children("tbody").children("tr").eq(3).children("td").eq(1).children("#organizationName").val(getPointSAByObj(mk,"organization_name"));
			jQuery("#stationIDcontent").children("#dataTable").children("tbody").children("tr").eq(4).children("td").eq(1).children("#sichen_addr").val(getPointSAByObj(mk,"station_addr"));
			jQuery("#stationIDcontent").children("#dataTable").children("tbody").children("tr").eq(3).children("td").eq(1).children("#orgidtag").val(getPointSAByObj(mk,"organization_id"));
		}				
	}//站点业务处理-end
}//clickMap方法结束
	...
	*/
	function setStationInfo(stationID,flag,centerPixX,centerPixY){
		if(flag==false){
			if(mapObj.getOverlayById(publicMapID)!=null){
				...
				if(getPointSAById(publicMapID,"pointType")=="0"){
					...
					if(getPointSAByObj(markerBefore,"showFlag")=="listAdd"||getPointSAByObj(markerBefore,"showFlag")=="searchCheckedAdd"||getPointSAByObj(markerBefore,"showFlag")=="searchListAdd"){
						markerBefore.option.imageUrl="../images/map/station2.png";
						//markerBefore.option.imageSize=new MSize(30,30);
					}else{
		...
		jQuery("#stationIDcontent").children("#lonlatTable").children("tbody").children("tr").eq(2).children("td").eq(0).children("#up_img").bind('click', function(event){
			add_Latitude();
		});

		jQuery("#stationIDcontent").children("#lonlatTable").children("tbody").children("tr").eq(3).children("td").eq(0).children("#left_img").bind('click', function(event){
			decrease_Longitude();
		});

		jQuery("#stationIDcontent").children("#lonlatTable").children("tbody").children("tr").eq(3).children("td").eq(1).children("#right_img").bind('click', function(event){
			add_Longitude();
		});

		jQuery("#stationIDcontent").children("#lonlatTable").children("tbody").children("tr").eq(4).children("td").eq(0).children("#down_img").bind('click', function(event){
			decrease_Latitude();
		});
	}
	...
	function checkForm() {
		...
		jQuery("#stationID").find("input").each(function(index){
			if(formatSpecialChar(jQuery(this).val())=="") {
			...
			if(jQuery(this).val().length>30&&jQuery(this).parent().prev().text()!="所属部门："){
				text = jQuery(this).parent().prev().text(); 
				alert(text+"长度过长！"); 
				jQuery(this).focus(); 
				pass = false; 
	...
	function checkLon(obj) {
  	  if(checkLonLat(jQuery("#stationIDcontent").children("#dataTable").children("tbody").children("tr").eq(1).children("td").eq(3).children("#site_longitude").val())&&checkLonLat(jQuery("#stationIDcontent").children("#dataTable").children("tbody").children("tr").eq(2).children("td").eq(3).children("#site_latitude").val())){
      	try{
            addPoiMarker();
          } catch(e) {
             ;
	...
	function saveMarker(thisMarker,id){
		if(id!=null){
			var lonValue=jQuery("#stationIDcontent").children("#dataTable").children("tbody").children("tr").eq(1).children("td").eq(3).children("#site_longitude").val();
			var latValue=jQuery("#stationIDcontent").children("#dataTable").children("tbody").children("tr").eq(2).children("td").eq(3).children("#site_latitude").val();
			var site_longitude=jQuery("#stationIDcontent").children("#dataTable").children("tbody").children("tr").eq(1).children("td").eq(3).children("#site_longitude").val();
			var site_latitude=jQuery("#stationIDcontent").children("#dataTable").children("tbody").children("tr").eq(2).children("td").eq(3).children("#site_latitude").val();
			...
			site_name = formatSpecialChar(site_name);
			...
			var sichen_addr=jQuery("#stationIDcontent").children("#dataTable").children("tbody").children("tr").eq(4).children("td").eq(1).children("#sichen_addr").val();
			sichen_addr = formatSpecialChar(sichen_addr);
			...
			var realLng=jQuery("#stationIDcontent").children("#dataTable").children("tbody").children("tr").eq(1).children("td").eq(3).children("#site_longitude").val();
			var realLat=jQuery("#stationIDcontent").children("#dataTable").children("tbody").children("tr").eq(2).children("td").eq(3).children("#site_latitude").val();
			var initLng=jQuery("#stationIDcontent").children("#dataTable").children("tbody").children("tr").eq(1).children("td").eq(3).children("#site_longitude").val(); //坐标初始经度
			var initLat=jQuery("#stationIDcontent").children("#dataTable").children("tbody").children("tr").eq(2).children("td").eq(3).children("#site_latitude").val();	//坐标初始纬度
			...
			station_addr = formatSpecialChar(station_addr);
	...
	function saveListData(mid){
		...
		jQuery('#checkbox_'+mid).val(mid+'#!#'+site_name+'#!#'+site_longitude+'#!#'+site_latitude+'#!#'+control_station+'#!#'+sichen_addr+'#!#'+orgidtag+'#!#'+organizationName);
...
function addStation(){
	...
	*/
	...
	function resetStation(){
		...
		jQuery("#stationIDcontent").children("#dataTable").children("tbody").children("tr").eq(1).children("td").eq(3).children("#site_longitude").val(getPointSAByObj(thisMarker,"initLng"));
		//getPointSAByObj(thisMarker,"initLat") thisMarker.option.initLat
		jQuery("#stationIDcontent").children("#dataTable").children("tbody").children("tr").eq(2).children("td").eq(3).children("#site_latitude").val(getPointSAByObj(thisMarker,"initLat"));
		addPoiMarker();
	}
	  ...
	  jQuery.ajax({
			...
			success: function(lonlatString) {
				...
				if(publicMapID!="BE66855BD543127FE0440019BB600AC6"){//点击操作触发
					...
					markerNew.rotation=0;
					markerNew.hasShadow=false;
					markerNew.labelOption=addMarkerLabel('',MConstants.BOTTOM_CENTER);
					var Marker="";
					Marker = new MMarker(new MLngLat(lonValue,latValue),markerNew);  
...
function add_Latitude(){
	...
	if(site_latitude==null||site_latitude=="")
		return false;
	jQuery("#stationIDcontent").children("#dataTable").children("tbody").children("tr").eq(2).children("td").eq(3).children("#site_latitude").val(accAdd(site_latitude,0.0001));
	addPoiMarker();
}
...
function decrease_Latitude(){
	...
	jQuery("#stationIDcontent").children("#dataTable").children("tbody").children("tr").eq(2).children("td").eq(3).children("#site_latitude").val(accSub(site_latitude,0.0001));
	addPoiMarker();
}
...
function add_Longitude(){
	...
	if(site_longitude==null||site_longitude=="")
		return false;
	jQuery("#stationIDcontent").children("#dataTable").children("tbody").children("tr").eq(1).children("td").eq(3).children("#site_longitude").val(accAdd(site_longitude,0.0001));
	addPoiMarker();
}
...
function decrease_Longitude(){
	...
	if(site_longitude==null||site_longitude=="")
		return false;
	jQuery("#stationIDcontent").children("#dataTable").children("tbody").children("tr").eq(1).children("td").eq(3).children("#site_longitude").val(accSub(site_longitude,0.0001));
	addPoiMarker();
}
