	...
	var galaList = jQuery('#galaList');
	 galaList.flexigrid({
		  ...
		  colModel : [ 
				...
				{display: 'id', name : 'ID', width : 120, sortable : true, align: 'center',hide:true,toggle:false,escape: true},
				...
				{display: '点火时间', name : 'ON_DATE', width : calcColumn(0.15,150,240), sortable : true, align: 'center'},
				{display: '熄火时间', name : 'OFF_DATE', width : calcColumn(0.15,150,240), sortable : true, align: 'center'},
		        ...
		        {display: '空调/加热器', name : '', width : 80, sortable : false, align: 'center',escape: true,process: reWriteLink}
		        ],
...
function reWriteLink(tdDiv,pid,row){
	var vin = row.cell[2];
	var stime = row.cell[5];
	var etime = row.cell[6];
	return '<a href="javascript:showDetail(\'' + vin + '\',\''+stime+'\',\''+etime+'\');">'+ '使用详情' + '</a>';
}

function showDetail(vin,stime,etime){
	var url="../runoil/showAirDetailPage.shtml?queryObj.vehicle_vin="+vin+"&queryObj.on_date="+stime+"&queryObj.off_date="+etime;
	art.dialog.open(url,{
		title:"使用详情",
		lock:true,
		width:595,
