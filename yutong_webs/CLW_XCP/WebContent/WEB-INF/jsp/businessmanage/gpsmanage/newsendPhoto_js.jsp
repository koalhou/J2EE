		...
		if(photolist[i] != null ){
			...
			// modify by jinp begin
				if(photolist[i].PHOTO_FALG == "1"){
					// 标记为超载时
					jQuery("#cancelOverload").css("display","block");
					jQuery("#initOverload").css("display","none");
					jQuery("#reoverload").css("display","none");
					jQuery("#PHOTO_CANCEL_FALG").attr("checked",false);
					<s:if test="#session.perUrlList.contains('111_3_1_20_3')">
					jQuery("#PHOTO_CANCEL_FALG").attr("disabled",false);
					</s:if>	
				    <s:else>
					jQuery("#PHOTO_CANCEL_FALG").attr("disabled",true);
				    </s:else>
					if(jQuery(window.parent.iframeshowArea).width() > 500){
						jQuery("#overloadOperator").html(photolist[i].FLAG_USER);
						jQuery("#overloadOperator").attr("title","");
					} else {
						var username = photolist[i].FLAG_USER;
						if(username != null && username.length > 3) {
							jQuery("#overloadOperator").html(username.substring(0,3) + "......");
							jQuery("#overloadOperator").attr("title",username);
						} else {
							jQuery("#overloadOperator").html(username);
						}
					}
					jQuery("#overloadOperatorTime").html(photolist[i].FLAG_TIME);
					//document.getElementById("PHOTO_FALG").value = photolist[i].PHOTO_FALG;
					//document.getElementById("PHOTO_CANCEL_FALG").checked = true;
				} else if(photolist[i].PHOTO_FALG == "0"){
					// 解除超载
					jQuery("#cancelOverload").css("display","none");
					jQuery("#initOverload").css("display","none");
					jQuery("#reoverload").css("display","block");
					jQuery("#PHOTO_RE_FALG").attr("checked",false);
					if(jQuery(window.parent.iframeshowArea).width() > 500){
						jQuery("#cancelOverloadOperator").html(photolist[i].RELI_USER);
						jQuery("#cancelOverloadOperator").attr("title","");
					} else {
						var username = photolist[i].RELI_USER;
						if(username != null && username.length > 3) {
							jQuery("#cancelOverloadOperator").html(username.substring(0,3) + "......");
							jQuery("#cancelOverloadOperator").attr("title",username);
						} else {
							jQuery("#cancelOverloadOperator").html(username);
						}
					}
					jQuery("#cancelOverloadOperatorTime").html(photolist[i].RELI_TIME);
					
				} else {
					// 相片初始状态
					jQuery("#cancelOverload").css("display","none");
					jQuery("#initOverload").css("display","block");
					jQuery("#reoverload").css("display","none");
					jQuery("#PHOTO_FALG").attr("checked",false);
...
function savePhotoInfo(){
	...
	// modify by jinp begin
	PHOTO_FALG = "";
	if(jQuery("#initOverload").css("display") == "block") {
		if(jQuery("#PHOTO_FALG").attr("checked")) {
			// 标记为超载
			PHOTO_FALG = 1;
		} else {
			PHOTO_FALG = "";
		}
	} else if(jQuery("#cancelOverload").css("display") == "block") {
		if(jQuery("#PHOTO_CANCEL_FALG").attr("checked")) {
			// 标记为解除超载
			PHOTO_FALG = 0;
		} else {
			PHOTO_FALG = "";
		...
		// 重新标记超载
		if(jQuery("#PHOTO_RE_FALG").attr("checked")) {
			// 标记为解除超载
			PHOTO_FALG = 1;
		} else {
			PHOTO_FALG = "";
	...
	**/
	var vin = document.getElementById("vin").value;
	var enterpriseid = document.getElementById("optionEnterpriseId").value;
	// modify by jinp end
	
	GPSDwr.SavePhotoInfo(PHOTO_ID,PHOTO_FALG,remark,userid,vin,enterpriseid,SavePhotoInfo_callback);
...
function styleControl(){
	...
	if(jQuery(window.parent.iframeshowArea).width() > 500){
		...
		jQuery('#photo_leftAreaDiv').width("493px");
		jQuery('#photo_leftAreaDiv').height("476px");

		jQuery('#photo_rightAreaDiv').width("160px");
		jQuery('#photo_rightAreaDiv').height("476px");

		jQuery('#PHOTO').width("436px");
		jQuery('#PHOTO').height("406px");

		jQuery('#PHOTO0').width("140px");
		jQuery('#PHOTO0').height("133px");
		jQuery('#PHOTO1').width("140px");
		jQuery('#PHOTO1').height("133px");
		jQuery('#PHOTO2').width("140px");
