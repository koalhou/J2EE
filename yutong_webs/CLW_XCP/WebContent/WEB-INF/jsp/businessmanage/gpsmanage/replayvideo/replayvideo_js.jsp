...
function showFactoryDiv(){
	...
	if(VidiconType == "HI"){
		...
		m_user = user;
		m_password = passwd;
		var Ctrl = document.getElementById("ocxyw");
		Ctrl.SetUIModeRemoteHistory();
		Ctrl.ShowVideoOnly(false);
		Ctrl.UseDefCtrlPane(true);
		Ctrl.SelectVehicle(document.getElementById('VEHICLE_LN').value);
		Ctrl.SetCameraName(1, '整车');
		Ctrl.SetCameraName(2, '路况');
		Ctrl.SetCameraName(3, '车门');
		Ctrl.SetCameraName(4, '司机');
	}
}
...
function checkDvrState(){
	var vin = document.getElementById("vin").value;	//alert(vin);
	GPSDwr.returnTipInfo(vin,returnDvrInfo_callback);
}

function returnDvrInfo_callback(date){
...
function selectgj(){
	...
	if(VidiconType == "HI"){
		...
		var str = "";
		for(var i = 0; i < waysstring.length; i++){
			var xmlresult = PlayOCX.PlaybackSearchFile(waysstring[i], 0, endtime+" 00:00:00",endtime+" 23:59:59");
			if(xmlresult == "" || xmlresult == "undefined"){
				...
				for (var j = 0; j < elements.length; j++) {
					...
					str = str+FileName+","+StartTime+","+StopTime+","+FileSize+","+FileType+","+waysstring[i]+"~";	                
				}	
			}
		}
		if(str != ""){
	...
	if(VidiconType == "DA"){
		...
		DAvideoresultstrs = "";
		for(var k = 0; k < waysstring.length; k++){
			var vid = m_vidiconId;
			var xmlresultDA = ocx.QueryRecords(vid,(waysstring[k]-1),endtime+" 00:00:00",endtime+" 23:59:59",0);
		}	
	}
	if(VidiconType == "YW"){
		var Ctrl = document.getElementById("ocxyw");
		Ctrl.ConnectMediaServer(m_szRegIP, m_iRegPort);
		Ctrl.SetUIModeRemoteHistory();
		
		Ctrl.ShowVideoOnly(false);
		Ctrl.UseDefCtrlPane(true);
		Ctrl.SelectVehicle("D00001");
...
function selectPlayFile(way,FileName,FileSize,starttime,endtime){
	var playtime = document.getElementById("line_end_time").value;
	if(VidiconType == "HI"){
		var PlayOCX = document.getElementById("PPVSHI1");
		if(0 > lConDevice){
			//针对宇通网络环境需调用该接口
		...
		for(var i = 0; i < waysstring.length; i++){
			var xmlresult = PlayOCX.PlaybackSearchFile(waysstring[i], 0, playtime+" 00:00:00",playtime+" 23:59:59");
			var playFileName="";
			var playStartTime="";
			var playStopTime="";
			if(xmlresult == "" || xmlresult == "undefined"){
				...
				var elements = xmldoc.getElementsByTagName("File");
				for (var j = 0; j < elements.length; j++) {
					playFileName = elements[j].getElementsByTagName("FileName")[0].firstChild.nodeValue;
					playStartTime = elements[j].getElementsByTagName("StartTime")[0].firstChild.nodeValue;
					playStopTime = elements[j].getElementsByTagName("StopTime")[0].firstChild.nodeValue;   
					if(FileName==playFileName){
						starttime=playStartTime;
						endtime=playStopTime;
					}
				}	
			}
	...
	var intervalValue=window.setInterval(function(){
		szInfo = PlayOCX.GetDeviceInfoOnServer(m_vidiconId, m_szRegIP, m_iRegPort, m_user, m_password);
...
function showdownloaddialog(pid,filename,starttime,endtime,way,size,vehicle_ln,VidiconType){
		...
		for(var i = 0; i < waysstring.length; i++){
			...
			if(xmlresult == "" || xmlresult == "undefined"){
				...
				for (var j = 0; j < elements.length; j++) {
					...
					playStopTime = elements[j].getElementsByTagName("StopTime")[0].firstChild.nodeValue;   
					if(filename==playFileName){
						starttime=playStartTime.replace(/\:/g,"").replace(/[ ]/g,"");
						endtime=playStopTime.replace(/\:/g,"").replace(/[ ]/g,"");
					}
				}	
			}
	...
	var filenamego=vehicle_ln+"_0"+way+waynorecode(way)+"_"+starttime.replace(/\-/g,"")+"_"+endtime.replace(/\-/g,"")+"_"+RndNum(6)+".mp4";
	art.dialog({
				...
				yesFn: function(){
					if(jQuery("#downloadfilename").val().length<5){
						...
						window.open ('../gps/downLoadHIReplayVideo.shtml?vehicle_vin='+m_vidiconId+'&pid='+pid+
				    		...
				    		'&VidiconType='+VidiconType+
				    		'&m_szRegIP='+m_szRegIP+
				    		'&m_iRegPort='+m_iRegPort+
				    		'&m_user='+m_user+
				    		'&m_password='+m_password,'_blank'); 
					}
			    },
			    noFn: true , 
	});
...
function styleControl(){
	if(jQuery(window.parent.iframeshowArea).width() > 500){
		...
		jQuery('#ocx').width("655px").height("400px");
		jQuery('.bDiv').height("164px");
		jQuery('.flexigrid').width("655px");
	}else{
		...
		jQuery('#ocx').width("500px").height("280px");
		jQuery('.bDiv').height("100px");
		jQuery('.flexigrid').width("500px");
	}
}
