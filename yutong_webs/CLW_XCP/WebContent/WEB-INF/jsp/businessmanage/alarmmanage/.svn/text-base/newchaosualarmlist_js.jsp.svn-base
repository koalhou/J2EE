	...
	var info="确认批量解除告警?";
  	if(confirm(info)) {
  		...
  		jQuery.ajax({
	   		  ...
	   		  data: {alarmids: alarmIdList,vins:vinList,alarmtypes:alarmtypeList,dealflags:dealfalgList,alramtimes:alarmtimelist,mouldid:"111_3_1_12_6"},	
	   		 success: function() {  
	   			searchList();
		      },
    		  error:function (){
...
jQuery(function() {
	...
	gala.flexigrid({
		...
		colModel : [{display: '经度' ,name : 'longitude',width : 80,hide:true,toggle:false},
		            ...
		            {display: '<input type="checkbox" id="checkallalarm" name="checkallalarm"  value="123" onclick="allchooseFlag(this)"/>',name : '',width : 25,sortable : false,<s:if test="#session.perUrlList.contains('111_3_1_12_6')">hide:false,</s:if><s:else>hide:true,toggle:false,</s:else>align: 'center',process:rewritecheckbox}, 
					{display: '车牌号', name : 'vehicle_ln', width : calcColumn(0.1,90,240), sortable : true, align: 'center',escape: true},
					{display: '告警类型', name : 'alarm_type_name', width : calcColumn(0.1,115,240), sortable : true, align: 'center'},
					{display: '处理状态', name : 'deal_flag', width : calcColumn(0.05,75,240), sortable : true, align: 'center',process: reWritedeal_flag},
					{display: '告警时间', name : 'alarm_time', width : calcColumn(0.15,100,240), sortable : true, align: 'center'},
...
function openScroll(){
	window.open("<s:url value="/busalarmpkg/openAlarmList.shtml" />", 'newwindow', 'height=1536px, width=1366px, top=0, left=0, toolbar=no, menubar=no, scrollbars=no, resizable=no,location=n o, status=no');
	//window.showModelessDialog("<s:url value="/busalarmpkg/openAlarmList.shtml" />", 'newwindow', 'dialogHeight=768px; dialogWidth=1366px; top=0; left=0; toolbar=no; menubar=no; scrollbars=no; resizable=no;location=no; status=no');
}
...
function exportReport(){
	...
	var idays = parseInt((enddate - startdate)/24/ 1000 / 60 / 60);
	if(idays> 15){
		alert("时间跨度大于16天,不可汇总");
		return;
	}
	if(confirm("确定要导出汇总信息么？")) {
		document.getElementById('alarmManage_form').action="<s:url value='/busalarm/exportChaosuReport.shtml' />";
		document.getElementById('alarmManage_form').submit();
	  }
}
</script>