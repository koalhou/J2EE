	...
	var info="确认批量解除告警?";
  	if(confirm(info)) {
  		...
  		jQuery.ajax({
	   		  ...
	   		  data: {alarmids: alarmIdList,vins:vinList,alarmtypes:alarmtypeList,dealflags:dealfalgList,alramtimes:alarmtimelist,mouldid:"111_3_1_12_7"},	
	   		 success: function() {  
	   			searchList();
		      },
    		  error:function (){
...
jQuery(function() {
	...
	gala.flexigrid({
		...
		colModel : [{display: '经度' ,name : 'longitude',width : 80,hide:true,toggle:false},
		            ...
		            {display: '<input type="checkbox" id="checkallalarm" name="checkallalarm"  value="123" onclick="allchooseFlag(this)"/>',name : '',width : 25,sortable : false,<s:if test="#session.perUrlList.contains('111_3_1_12_7')">hide:false,</s:if><s:else>hide:true,toggle:false,</s:else>align: 'center',process:rewritecheckbox}, 
