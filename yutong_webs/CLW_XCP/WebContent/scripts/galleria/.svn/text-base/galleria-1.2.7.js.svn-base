...
Galleria.prototype = {
    ...
    */
    ...
    init: function( target, options ) {
        ...
        // set show index
        if ( options && typeof options.showIndex === 'number' ) {
        	this._options.show = options.showIndex;
        }
    ...
    _createThumbnails : function() {
        ...
        var i,
            ...
            onThumbEvent = function( e ) {//点击小缩略图时触发
				...
				//alert(self._data[index].phId);
				getPhotoInfo(self._data[index]);
                e.preventDefault();
            },
...
this.prependChild( 'info', 'myElement' );
    ...
    */
    ...
    next : function() {//点击下一张时触发
        if ( this.getDataLength() > 1 ) {	
        	// Fan Lisong 2014.07.10
        	// for preview all image
        	if (jQuery.type(nextPage)=='function' && nextPage(this.getIndex())) {
        		return this;
        	}
            this.show( this.getNext(), false );
			//alert(this._data[this.getIndex()].phId);
			getPhotoInfo(this._data[this.getIndex()]);
        } else {
        	// Fan Lisong 2014.08.21
        	// for preview all image
        	if (jQuery.type(nextPage)=='function' && nextPage(this.getIndex())) {
        		return this;
        	}
    ...
    */
    ...
    prev : function() {//点击上一张时触发
        if ( this.getDataLength() > 1 ) {
        	// Fan Lisong 2014.07.10
        	// for preview all image
        	if (jQuery.type(prevPage)=='function' && prevPage(this.getIndex())) {
        		return this;
        	}
    ...
    /**
        Fan Lisong 2014.07.16
        for preview all image
        	
        Shows the last image in line

        @returns Instance
    */

    last : function() {//点击上一张时触发
        if ( this.getDataLength() > 1 ) {
        	var showIndex = this.getDataLength() - 1;
            this.show( showIndex , true );
			getPhotoInfo(this._data[showIndex]);
        }
        return this;
    },
            ...
            onload = (function( self, callback, src ) {
                ...
                return function() {
                    ...
                    var complete = function() {
                        ...
                        // save the original size update by fanxy
                        self.original = size || {
                            //height: this.height,
                        	height: 240,
                            //width: this.width
