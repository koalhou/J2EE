...
function CheckTree(myName){
	this.myName=myName;this.root=null;
...
CheckTree.prototype.init=function(){
	with(this){
		if(!document.getElementById)return;root=document.getElementById('tree-'+myName);
		if(root){
			...
			root.treeObj=this;root.setBoxStates=setBoxStates;
...
CheckTree.prototype.click=function(e){
	with(this){
		e=e||window.event;var elm=e.srcElement||e.target;
...
function setBoxStates(e,routingDown,countOnly){
	with(this){
		if(!this.childNodes)return;e=e||window.event;
		...
		if(countElm){
			...
			if(boxCount){
				...
				if (boxCount > 0) parBox.checked = true;
			}
		}
	}
};