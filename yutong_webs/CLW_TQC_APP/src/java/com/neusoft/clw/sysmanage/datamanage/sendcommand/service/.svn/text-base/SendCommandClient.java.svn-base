...
public class SendCommandClient {
     ...
     * @return
     */
    public String sendKeyMonitorCommand(String vehicle_vin, String userid, String interval, String duration, String msgid, String batch_id) {
        SendCommand sendcommand = new SendCommand();
        sendcommand.setSendcommandid(msgid);
        String recode = "7001";
        boolean setCore_id_b = false;
        ...
        if (MemData.getAppcfgList() != null) {
            if (MemData.getAppcfgList().get(0) != null) {
                sendcommand.setCore_id(MemData.getAppcfgList().get(0).getCore_id());
                setCore_id_b = true;
            } else {
                log.warn(msgid + ",MemData.getAppcfgList().get(0)为空,reload config!");
                initParameter();
            }
        } else {
            log.warn(msgid + ",MemData.appcfgList为空,reload config!");
        ...
        List < VehcileInfo > vehlist = null;
        try {
            vehlist = service.getObjects("SendCommand.getSIMCardNumber", vehicle_vin);
        } catch (Exception e) {
            log.error(msgid + ",监听指令下发查询SIM卡出错:", e);
            return recode;
        }
        ...
        String sim_card_number = "";
        if (vehlist.get(0).getSim_card_number() != null && vehlist.get(0).getSim_card_number() != "") {
            sim_card_number = vehlist.get(0).getSim_card_number();
        } else {
            log.warn(msgid + ",不存在对应Sim_card_number信息!");
            return recode;
        ...
        if ("0".equals(interval)) {
            packet.append(format(Integer.toHexString(Integer.parseInt(interval)), 4));
            ...
            packet.append(format(Long.toHexString(Long.parseLong(duration)), 8));
        }
     ...
     * @return
     */
    public String sendAlaramOffCommand(String vehicle_vin, String userid, String alarmtype, String msgid, String alarm_id, String remark,String batch_id) throws Exception {
        SendCommand sendcommand = new SendCommand();
        sendcommand.setSendcommandid(msgid);
        String recode = "7001";
        boolean setCore_id_b = false;
    ...
    public String doPost(String strSend, String msgid)
        ...
        String recode = "7002";
        try {
            log.info(msgid + ",请求xml内容:" + OlxDocument.Factory.parse(StringUtils.trim(strSend)).xmlText());
            ...
            if (200 == nStatusCode) {
                ...
                log.info(OlxDocument.Factory.parse(strReceive.toString())
                        .toString());
                // 解析响应
                OlxDocument doc = OlxDocument.Factory.parse(strReceive.toString());
                Olx olx = doc.getOlx();
