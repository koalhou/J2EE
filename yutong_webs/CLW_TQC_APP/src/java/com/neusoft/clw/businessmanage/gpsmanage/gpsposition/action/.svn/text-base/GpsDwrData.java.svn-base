...
import com.neusoft.clw.common.util.Distance;
...
public class GpsDwrData extends PaginationAction {
    ...
    protected  Map < String, Object > mapVin = new HashMap < String, Object >();
            ...
            if (list != null && list.size() > 0) {
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    if (tv.getTERMINAL_TIME() == null
                        ...
                        } else if (tv.getOVERLOAD_FLAG() != null
                                && !tv.getOVERLOAD_FLAG().equals("")) {
                            if (tv.getOVERLOAD_FLAG().equals("1")) {
                                tv.setColor("r");// 红色
                            }
                        }
    ...
    public List < TerminalViBean > returnTipInfo(String vin) {
        ...
        Map < String, Object > map = new HashMap < String, Object >();
        map.put("VEHICLE_VIN", vin);
        map.put("START_DATE", DateUtil.getCurrentDay());
        try {
            // List<TerminalViBean> list = service.getObjects(
            // "GPSNEW.getTipInfoByVin", map);
            // TerminalViBean tv = list.get(0);
            TerminalViBean list = (TerminalViBean) service.getObject("GPSNEW.getTipInfoByVin", map);
            AlarmRecode ar = new AlarmRecode();
            if (list != null) {
    ...
    public List < TerminalViBean > returnTipInfoNew(String vin,String lon,String lat) {
        ...
        try {            
            ...
            if (list != null) {
                ...
                List < TerminalViBean > l = new ArrayList<TerminalViBean>();
    ...
    public Map getVehcileLineList(String vin, String starttime, String endtime) {
        ...
        if (list != null && list.size() > 0) {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                //定位无效的点处理
                //对于在轨迹上定位无效的点，
                //如果是线路开始无定位，就一直显示最早定位的那个点
                //如果是在线路中定位突然失效，就显示失效前最晚的那个点
                if (tv.getTERMINAL_TIME() == null || tv.getTERMINAL_TIME().equals("")) {
                    continue;
                } else if (tv.getLONGITUDE() == null || tv.getLONGITUDE().equals("") || tv.getLATITUDE() == null || tv.getLATITUDE().equals("")) {
                    continue;
                } else if ( tv.getLONGITUDE().equals("FFFF") || tv.getLATITUDE().equals("FFFF")){
                	if(!emptyflag){
                		tv.setLONGITUDE(lastLon);
                		tv.setLATITUDE(lastLat);
                	}
                } else{
                	if(emptyflag){
                		for(TerminalViBean tvb : newlist){
                			tvb.setLATITUDE(tv.getLATITUDE());
                			tvb.setLONGITUDE(tv.getLONGITUDE());
                		}
                		emptyflag = false;
                	}
                }
                
                lastLat = tv.getLATITUDE();
                lastLon = tv.getLONGITUDE();
                
                if(tv.getALARM_TYPE_ID() != null && !tv.getALARM_TYPE_ID().equals("")){
                	tv.setColor("r");// 红色
                }
                ...
                /*
                 * if((tv.getALARM_BASE_INFO() != null &&
                 * !tv.getALARM_BASE_INFO().equals("")) ||
                 * (tv.getALARM_EXT_INFO() != null &&
                 * !tv.getALARM_EXT_INFO().equals("")) || (tv.getOVERLOAD_FLAG()
                 * != null && !tv.getOVERLOAD_FLAG().equals(""))){
                 * errorlist.add(ev); }
                 */
    ...
    public boolean SavePhotoInfo(String id, String flag, String remark,String flag_user) {
    	//MDC.put("modulename", "[cheliangMonitor-paizhao]");
    ...
    public String postKeyMonitor(String vehicle_vin, String userid, String interval, String duration) {
        String batch_id = UUIDGenerator.getUUID32();
        String msgid = UUIDGenerator.getUUID32();
        log.info("msgid:" + msgid + ";vin:" + vehicle_vin + ";interval:" + interval + ";userid:" + userid + ";duration:" + duration);
        String returnvalue = isOnoff(vehicle_vin, msgid);
        if (Constants.IS_ON_LINE.equals(returnvalue)) {
            returnvalue = sendCommandClient.sendKeyMonitorCommand(vehicle_vin, userid, interval, duration, msgid, batch_id);
        }
        log.info("msgid:" + msgid + ";returnvalue:" + returnvalue);
        return returnvalue;
    }
    ...
    public String getTabCount(String orgid) {
        ...
        try {
            ...
            /*TabCount tabcount = (TabCount) service.getObject("AlarmManage.getTabAlarmCount1", alarmmanage);
            ...
            countstr.append(",");
            countstr.append(tabcount.getTab5count());
            countstr.append(",");
            countstr.append(tabcount.getTab6count());*/
            
            TabCount tabcount2 = (TabCount) service.getObject("AlarmManage.getTabAlarmCount2", alarmmanage);
            TabCount tabcount3 = (TabCount) service.getObject("AlarmManage.getTabAlarmCount3", alarmmanage);
            TabCount tabcount4 = (TabCount) service.getObject("AlarmManage.getTabAlarmCount4", alarmmanage);
            TabCount tabcount5 = (TabCount) service.getObject("AlarmManage.getTabAlarmCount5", alarmmanage);
            TabCount tabcount6 = (TabCount) service.getObject("AlarmManage.getTabAlarmCount6", alarmmanage);
            countstr.append(tabcount2.getTab1count());
            countstr.append(",");
            countstr.append(tabcount2.getTab2count());
            countstr.append(",");
            countstr.append(tabcount3.getTab3count());
            countstr.append(",");
            countstr.append(tabcount6.getTab4count());
            countstr.append(",");
            countstr.append(tabcount4.getTab5count());
            countstr.append(",");
            countstr.append(tabcount5.getTab6count());
            
            Map<String, String> map = new HashMap<String, String>();
//			map.put("vehicle_vin", vin);
			map.put("enterprise_id", orgid);
			map.put("entId", orgid);
			map.put("start_time", DateUtil.getPreNDay(-2));
			map.put("end_time", DateUtil.getCurrentDay() + " 23:59:59");
			map.put("isValid", "0");
			int totalCount = service.getCount("ZsptFtlyinfoNew.getStealAlarmListPageCount1", map);
			countstr.append(",");
     ...
     * 轨迹回访
     */
    public int setRoutePoint(String sdate, String edate, String routeid,String vin) {
    	try {
			Map<String,Object> map = new HashMap<String,Object>();
			map.put("s_date", sdate);
			map.put("e_date", edate);
			map.put("route_id", routeid);
			map.put("vin", vin);
			map.put("out_point_ref", null);
			map.put("out_site_ref", null);
			service.getObject("GPSNEW.add_route_point_", map);
			
			//查看线路对应的站点	循环站点	搜索每个站点对应线路最近的点更新线路点
			//站点一定会有如果不报错判断站点	报错站点返回0
			if (map.get("out_site_ref")!=null) {
                ArrayList<ClwTqcEmpPreline> res = (ArrayList<ClwTqcEmpPreline>) map.get("out_point_ref");
                ArrayList<Site> sites = (ArrayList<Site>) map.get("out_site_ref");
                
                Double[] dou = new Double[res.size()];
                for(Site si :sites) {
                	for(int i=0;i<res.size();i++) {
                		ClwTqcEmpPreline point = res.get(i);
                		dou[i] = Distance.GetDistance(Double.valueOf(point.getLongitude()),Double.valueOf(point.getLatitude()),Double.valueOf(si.getSite_longitude()),Double.valueOf(si.getSite_latitude()));
                	}
                	//查出数组中最小值的index下标	比如6
                	int index_ = checkminsize(dou);
                	//更新它将si.getSite_id()更新进clw_tqc_ygb.clw_tqc_emp_preline_t表
                	Map<String,Object> mapupdate = new HashMap<String,Object>();
                	mapupdate.put("note_id", res.get(index_).getNoteId());
                	mapupdate.put("site_id", si.getSite_id());
                	mapupdate.put("latitude", si.getSite_latitude());
                	mapupdate.put("longitude", si.getSite_longitude());
                	service.update("GPSNEW.update_route_point_site", mapupdate);
                }
                List<ClwTqcEmpPreline> list = (List<ClwTqcEmpPreline>) service.getObjects("GPSNEW.getRoutePointById", map);
                ClwTqcEmpPreline pointBase = list.get(0); 
                int final_index = 1;
                String pointIds = "";
                for(int i=1;i<list.size()-2;i++){
                	ClwTqcEmpPreline pointThis = list.get(i);
                	ClwTqcEmpPreline pointNext = list.get(i+1);
                	Double min_distance_flag = Distance.GetDistance(pointBase.getLongitude(), pointBase.getLatitude(), pointThis.getLongitude(), pointThis.getLatitude());
                	Double angle1 = Distance.GetAngle(pointBase.getLongitude(), pointBase.getLatitude(), pointThis.getLongitude(), pointThis.getLatitude());
                	Double angle2 = Distance.GetAngle(pointThis.getLongitude(), pointThis.getLatitude(), pointNext.getLongitude(), pointNext.getLatitude());
                	//如果是站点，则必为轨迹点，或者已经超过基准点100米，则认为是轨迹点
                	if(pointThis.getIsSite() == null && min_distance_flag < 100){
                		//如果当前点距离基准点不足20米，或者当前点与基准点的斜率和当前点与
                		//下一点的斜率夹角不足15度，则认为是冗余点
                		if(min_distance_flag  < 20 || Math.abs(angle1 - angle2) < 15){
                			pointIds += pointThis.getNoteId() + ",";
                		}else{
                			pointBase = list.get(i);
                		}
                	}else{
                		pointBase = list.get(i);
                	}	
            	...
            	mapTemp.put("points", pointIds.substring(0,pointIds.length() - 1));
            	service.update("GPSNEW.update_surplus_route_point", mapTemp);
                return 1;
            }
			return 0;
		} catch (BusinessException e) {
    ...
    private int checkminsize(Double[] dou) {
    	int index_size = 0;
    	Double temp = dou[0];
     ...
     * @return
     */
    public Map< String, Object > getSimpleRecord(String vin, String starttime, String endtime){
    	Map<String,String> param=new HashMap<String,String>();
    	param.put("vin", vin);
    	param.put("startTime", starttime);
    	param.put("endTime", endtime);
    	List<SimpleRecord> list=null;
		try {
			list = this.service.getObjects("GPSNEW.getRecordByVin", param);
		} catch (BusinessException e) {
			e.printStackTrace();
		}
		//轨迹偏移
		gpsUtil gpsUtil = new gpsUtil();
        //newlist = gpsUtil.getpost(newlist);
		gpsUtil.getMapLonLat(list);
    	Map resultMap = new HashMap < String, Object >();
        resultMap.put("data", list);
        return resultMap;
    }
    /**
     * 本方法对“车辆监控--车辆筛选模块”进行查询
     * @param vins
     * @return
     */
