...
public class PasswordConfirmAction extends PaginationAction {
    ...
    public boolean checkDriverLicense(String driverLicense) {
        ...
        driverInfo.setDriver_license(driverLicense.trim());
        driverInfo.setOrganization_id(user.getOrganizationID());
        try {
            int cnt = service.getCount("DriverManage.checkDriverLicense", driverInfo);
            if (cnt == 0) {
                return false;
            } else {
                return true;
            ...
            log.error("验证驾驶员证号唯一性DWR异常发生，异常原因：" + e.getMessage());
        }
     ...
     * @return
     */
    public boolean checkDriverLicense1(String driverLicense) {
        boolean ret = false;
        WebContext ctx = WebContextFactory.get();
        HttpServletRequest request = ctx.getHttpServletRequest();
        UserInfo user = (UserInfo) request.getSession().getAttribute(Constants.USER_SESSION_KEY);
    ...
    public boolean checkDriverCard(String driverCard) {
        ...
        UserInfo user = (UserInfo) request.getSession().getAttribute(Constants.USER_SESSION_KEY);
        DriverInfo driverInfo=new DriverInfo();
        driverInfo.setDriver_card(driverCard.trim());
        driverInfo.setOrganization_id(user.getOrganizationID());
        
        try {
            int cnt = service.getCount("DriverManage.checkDriverCard", driverInfo);
            if (cnt == 0) {
                return false;
            } else {
                return true;
     ...
     * @return
     */
    public boolean checkDriverCard1(String driverCard) {
        boolean ret = false;
        WebContext ctx = WebContextFactory.get();
        HttpServletRequest request = ctx.getHttpServletRequest();
        UserInfo user = (UserInfo) request.getSession().getAttribute(Constants.USER_SESSION_KEY);
