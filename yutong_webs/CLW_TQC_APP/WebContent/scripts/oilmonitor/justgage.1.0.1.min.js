...
JustGage.prototype.refresh = function(b,freshColor,freshTitle,freshMaxOil) {
	originalVal = b;
	this.config.max = freshMaxOil;
	//this.config.min = 0;
	...
	this.canvas.getById(this.config.id + "-txttitle").attr({text : freshTitle});
	this.canvas.getById(this.config.id + "-txtvalue").attr({
		text : originalVal
	});
	this.canvas.getById(this.config.id + "-txtmax").attr({text : freshMaxOil});
	var color = getColorForPercentage((parseFloat(b) - this.config.min) / (this.config.max - this.config.min), this.config.levelColors,
			this.config.levelColorsGradient);
	//	/(Number(freshMaxOil) - this.config.min), this.config.levelColors,
