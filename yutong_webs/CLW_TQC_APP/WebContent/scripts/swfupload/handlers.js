function fileQueueError(file, errorCode, message) {
	try {
		var imageName = "<font color='red'>文件错误</font>";
		var errorName = "";
		...
		addReadyFileInfo(file.id,file.name,imageName,"无法填加");
...
/**
 * 当文件选择对话框关闭消失时，如果选择的文件成功加入填加队列，
 * 那么针对每个成功加入的文件都会触发一次该事件（N个文件成功加入队列，就触发N次此事件）。
 * @param {} file
 * id : string,			    // SWFUpload控制的文件的id,通过指定该id可启动此文件的填加、退出填加等
...
function fileQueued(file){
	addReadyFileInfo(file.id,file.name,"准备填加文件：");
}
...
function uploadComplete(file) {
	try {
		...
		if (this.getStats().files_queued > 0) {
			...
			//progress.setStatus("<font color='green'>所有文件填加完毕!</b></font>");
			progress.toggleCancel(false);
			
			var tal = document.getElementById("infoTable");
			for(var i=tal.children.length; i>0; i--)  
			{  
				tal.children[i-1].parentNode.removeChild(tal.children[i-1]);
			}
			showUploadedFiles();
		}
	} catch (ex) {
...
function uploadError(file, errorCode, message) {
	var imageName =  "<font color='red'>文件填加出错!</font>";
	var progress;
	try {
		switch (errorCode) {
		case SWFUpload.UPLOAD_ERROR.FILE_CANCELLED:
			try {
				...
				progress.setStatus("<font color='red'>取消填加!</font>");
				progress.toggleCancel(false);
			}
			catch (ex1) {
				this.debug(ex1);
		...
		case SWFUpload.UPLOAD_ERROR.UPLOAD_STOPPED:
			try {
				progress = new FileProgress(file,  this.customSettings.upload_target);
				progress.setCancelled();
				progress.setStatus("<font color='red'>停止填加!</font>");
...
function FileProgress(file, targetID) {
	...
	if (!this.fileProgressWrapper) {
		...
		//progressText.appendChild(document.createTextNode("填加文件: "+file.name));
		...
		document.getElementById(targetID).style.height = "10px";
		document.getElementById(targetID).appendChild(this.fileProgressWrapper);
		fadeIn(this.fileProgressWrapper, 0);

	} else {
		this.fileProgressElement = this.fileProgressWrapper.firstChild;
		//this.fileProgressElement.childNodes[1].firstChild.nodeValue = "填加文件: "+file.name;
	}
