...
// 		jQuery('#deal_flag').val(0);
		jQuery("#piliangchuli").css("display","none");   //油量告警不能要批量处理
...
function senddealcommand(){
	...
	for(var i=0; i<alarmChoice.length; i++) {
	       if(alarmChoice[i].checked==true) {
	    	   ...
	    	   var vin=get_cell_text(alarmChoice[i].value, 18);
	    	   var alarmtypeid=get_cell_text(alarmChoice[i].value, 17);
	    	   var alarmtime=get_cell_text2(alarmChoice[i].value, 5);
	    	   if(dealflag==1){
	    	   }else{
	    		   alarmIdList += get_cell_text(alarmChoice[i].value,16)+",";
	    		   vinList+=vin+",";
...
//                     {display: '方向', name : 'direction', width : 30,hide:true,toggle:false},	
					...
					{display: '告警类型', name : 'oilbox_state', width : calcColumn(0.04,100,240), sortable : true, align: 'center'},
					{display: '处理状态', name : 'is_valid', width : calcColumn(0.05,75,240), sortable : true, align: 'center'},//,process: reWritedeal_flag
					{display: '告警时间', name : 'report_time', width : calcColumn(0.12,100,240), sortable : true, align: 'center'},
					{display: '变动油量(L)', name : 'add_Oill', width : calcColumn(0.1,85,240), sortable : true, align: 'center',escape: true},			
					{display: '地理位置' , name : 'zonename', width : 350, sortable :false, align: 'center'},					
					...
					{display: '驾驶员' , name : "NLSSORT(driver_name,'NLS_SORT = SCHINESE_PINYIN_M')", width : 80, sortable :false, align: 'center',toggle:false,escape: true},
					...
					{display: '' , name : 'operate_time', width : 80, sortable :false, align: 'center',hide:true,toggle:false,escape: true}
					],	    		
...
function rowstandbyclick1(rowData){
	...
	var flag = jQuery(rowData).data("is_valid");
	if(flag=='未处理'){
		jQuery('#chuli').css('display','block');
	} else {
		jQuery('#chuli').css('display','none');
...
// 	jQuery("#alarmTime").html(jQuery(rowData).data("alarm_time"));
	jQuery("#alarmtypename1").val(jQuery(rowData).data("oilbox_state"));
	jQuery("#changeOil").val(jQuery(rowData).data("add_Oill"));
// 	jQuery("#alarmProPerson").val(jQuery(rowData).data("user_name"));
	jQuery("#yijianneirong").val(jQuery(rowData).data("operate_desc"));
	
// 	jQuery("#alarmDeriver").val(jQuery(rowData).data("driver_name"));
// 	jQuery("#countPerson").val(jQuery(rowData).data("stu_num"));
	jQuery("#alarmAddr").val(jQuery(rowData).data("zonename"));
	jQuery("#messagetime").html(jQuery(rowData).data("report_time"));
...
// 	jQuery("#chengce1").val(jQuery(rowData).data("stu_num"));
	//隐藏字段赋值
	jQuery("#chulialarmid").val(jQuery(rowData).data("ftly_id"));
	jQuery("#chulialarmtime").val(jQuery(rowData).data("report_time"));
	jQuery("#chulialarmtypeid").val(jQuery(rowData).data("oilboxState"));
	jQuery("#chulilon").val(jQuery(rowData).data("longitude"));
	jQuery("#chulilat").val(jQuery(rowData).data("latitude"));
	jQuery("#chulivin").val(jQuery(rowData).data("vin_code"));
	...
	if(flag == '未处理'){
		...
		jQuery('#yijianneirong').html(encodeHtml(jQuery(rowData).data("operate_desc")));
	}
...
function exportAlarm(){
	...
	if(confirm("确定要导出告警信息么？")) {
		...
		var sortname = jQuery(gala).flex_sortname();
		var sortorder = jQuery(gala).flex_sortorder();
		//document.getElementById('alarmManage_form').action="<s:url value='/ftlyInfoNew/exportOilalarm.shtml' />?sortname="+sortname;
		document.getElementById('alarmManage_form').action="<s:url value='/ftlyInfoNew/exportOilalarm.shtml' />?sortname="+sortname+"&sortorder="+sortorder;
