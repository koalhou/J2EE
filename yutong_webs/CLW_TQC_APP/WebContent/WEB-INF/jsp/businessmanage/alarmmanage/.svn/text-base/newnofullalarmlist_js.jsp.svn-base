...
function qinli(){
	...
	jQuery("#zhandian").val('');
	jQuery("#zhijishijian").val('');
	jQuery("#siji").val('');
	jQuery("#jihuashijian").val('');
	jQuery("#tiqianshijian").val('');
...
jQuery(function() {
	gala = jQuery('#gala');
	gala.flexigrid({
		url: '<s:url value="/busalarmpkg/newnofulltalarmlist.shtml" />',
		dataType: 'json',
		height: '2000',
		width:'800',
		colModel : [
		            ...
		            {display: '班车号', name : 'vehicle_code', width : calcColumn(0.05,8,1), sortable : true, align: 'center',escape: true},
					{display: '车牌号', name : 'vehicle_ln', width : calcColumn(0.05,8,1), sortable : true, align: 'center',escape: true},
					{display: '告警类型', name : 'alarm_type_name', width : calcColumn(0.07,83,150), sortable : true, align: 'center',escape: true},
					{display: '处理状态', name : 'operate_flag', width : calcColumn(0.05,75,100), sortable : true, align: 'center',process: reWritedeal_flag},
					{display: '行驶线路', name : 'route_name', width : calcColumn(0.1,105,200), sortable : true, align: 'center'},
					{display: '告警时间', name : 'alarm_date', width : calcColumn(0.1,105,200), sortable : true, align: 'center'},
					{display: '非时发车站点', name : 'site_name', width : calcColumn(0.1,15,240), sortable : true, align: 'center'},
					{display: '计划发车时间', name : 'send_time', width : calcColumn(0.1,105,200), sortable : true, align: 'center'},
					{display: '实际发车时间', name : 'terminal_time', width : calcColumn(0.1,105,200), sortable : true, align: 'center'},
					{display: '发车时间差(分)', name : 'time_dis', width : calcColumn(0.04,15,20), sortable : false, align: 'center'},
					{display: '空余座位数', name : 'siter_left', width : calcColumn(0.04,15,20), hide: true,sortable : true, align: 'center',escape: true,toggle: false},
					{display: '驾驶员', name : "NLSSORT(driver_name,'NLS_SORT = SCHINESE_PINYIN_M')", width : calcColumn(0.03,50,40), sortable : true, align: 'center'},
					{display: '联系方式', name : 'driver_tel', width : calcColumn(0.05,50,40), hide: true, sortable : true, align: 'center',escape: true,toggle: false},
...
// 		jQuery("#zhandian").val(jQuery(rowData).data("site_name"));
// 		jQuery("#alarmTime").val(jQuery(rowData).data("alarm_date"));
		
		jQuery("#jihuashijian").val(jQuery(rowData).data("send_time"));
// 		jQuery("#tiqianshijian").val(jQuery(rowData).data("terminal_time"));
		...
		jQuery("#zhijishijian").val(jQuery(rowData).data("terminal_time"));
		jQuery("#tiqianshijian").val(encodeHtml(jQuery(rowData).data("time_dis")) + "（分）");
...
// 		jQuery("#sijidianhua").val(jQuery(rowData).data("driver_tel"));
		...
		jQuery("#chulialarmtime").val(jQuery(rowData).data("alarm_date"));
...
function exportAlarm() {
	...
	if(confirm("确定要导出告警信息么？")) {
		...
		document.getElementById('alarmManage_form').action="<s:url value='/busalarm/exportnon-timealarm.shtml' />";
