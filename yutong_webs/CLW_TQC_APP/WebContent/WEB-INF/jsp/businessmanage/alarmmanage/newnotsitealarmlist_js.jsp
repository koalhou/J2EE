...
jQuery(function() {
	gala = jQuery('#gala');
	gala.flexigrid({
		url: '<s:url value="/busalarmpkg/newnotsitetalarmlist.shtml" />',
		dataType: 'json',
		height: '2000',
		width:'800',
		colModel : [
					...
					{display: '行驶线路', name : 'route_name', width : calcColumn(0.08,75,100), sortable : true, align: 'center'},
					{display: '告警时间', name : 'alarm_date', width : calcColumn(0.08,105,200), sortable : true, align: 'center'},
					{display: '告警位置', name : 'position', width : calcColumn(0.25,15,240), sortable : false, align: 'center'},
					{display: '上一站点', name : 'up_site', width : calcColumn(0.06,15,240), hide: false,sortable : true, align: 'center'},
					{display: '下一站点', name : 'down_site', width : calcColumn(0.06,15,240), hide: true,sortable : true, align: 'center'},
					{display: '下一站点', name : 'site_name', width : calcColumn(0.06,105,200), hide: false,sortable : true, align: 'center',escape: true,toggle: false},
	...
	function rowstandbyclick3(rowData){
		...
		jQuery("#upSite").val(jQuery(rowData).data("up_site"));
		jQuery("#downSite").val(jQuery(rowData).data("site_name"));
		...
		jQuery("#shijian").val(encodeHtml(jQuery(rowData).data("terminal_time")));
		jQuery("#diliweizhi").val(encodeHtml(jQuery(rowData).data("position")));
		jQuery("#zuijinzhandian").val(encodeHtml(jQuery(rowData).data("down_site")));
...
function exportAlarm() {
	...
	if(confirm("确定要导出告警信息么？")) {
		...
		document.getElementById('alarmManage_form').action="<s:url value='/busalarm/exportopendooralarm.shtml' />";
