...
function qinli(){
	...
	jQuery("#xingming").val('');
	jQuery("#xuexiao").val('');
	jQuery("#yijianneirong").val('');

	jQuery("#routeName").val('');
	jQuery("#kongzuo").val('');
	jQuery("#zhandian").val('');
	jQuery("#siji").val('');
	jQuery("#hezairenshu").val('');
	jQuery("#shizairenshu").val('');
...
jQuery(function() {
	gala = jQuery('#gala');
	gala.flexigrid({
		url: '<s:url value="/busalarmpkg/newnofulltalarmlist1.shtml" />',
		dataType: 'json',
		height: '2000',
		width:'800',
		colModel : [
		            ...
		            {display: '班车号', name : 'vehicle_code', width : calcColumn(0.02,8,1), sortable : true, align: 'center',escape: true},
					{display: '车牌号', name : 'vehicle_ln', width : calcColumn(0.04,8,1), sortable : true, align: 'center',escape: true},
					{display: '告警类型', name : 'alarm_type_name', width : calcColumn(0.06,83,150), sortable : true, align: 'center',escape: true},
					{display: '处理状态', name : 'operate_flag', width : calcColumn(0.04,75,100), sortable : true, align: 'center',process: reWritedeal_flag},
					{display: '告警时间', name : 'alarm_date', width : calcColumn(0.08,105,200), sortable : true, align: 'center'},
					{display: '行驶线路', name : 'route_name', width : calcColumn(0.1,105,200), sortable : true, align: 'center'},
					{display: '未满发车站点', name : 'site_name', width : calcColumn(0.1,15,240), sortable : true, align: 'center'},
					{display: '核载人数(个)', name : 'standard_cnt', width : calcColumn(0.06,15,140), sortable : true, align: 'center'},
					{display: '实载人数(个)', name : 'real_cnt', width : calcColumn(0.06,15,140), sortable : true, align: 'center'},
					{display: '空余座数(个)', name : 'empty_num', width : calcColumn(0.06,15,140), sortable : true, align: 'center'},
					{display: '驾驶员', name : "NLSSORT(driver_name,'NLS_SORT = SCHINESE_PINYIN_M')", width : calcColumn(0.03,50,80), sortable : true, align: 'center'},
...
// 		jQuery("#alarmTime").val(jQuery(rowData).data("alarm_date"));
		...
		var tmpPerson = jQuery(rowData).data("user_name");
		if(tmpPerson.length <= 1){
			jQuery("#alarmProPerson").val("无");	
		} else {
			jQuery("#alarmProPerson").val(tmpPerson);
		}
		
		jQuery("#shizairenshu").val(jQuery(rowData).data("real_cnt"));
		...
		jQuery("#kongzuo").val(encodeHtml(jQuery(rowData).data("empty_num")));
		jQuery("#hezairenshu").val(encodeHtml(jQuery(rowData).data("standard_cnt")));
