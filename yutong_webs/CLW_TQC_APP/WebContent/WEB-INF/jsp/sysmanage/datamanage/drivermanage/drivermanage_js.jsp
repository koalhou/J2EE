...
var gala='';
	jQuery(function() {
		 ...
		 gala.flexigrid({
		  ...
		  colModel : [ 
				...
				{display: '<s:text name="driverinfo.driver_name" />', name : "NLSSORT(DRIVER_NAME,'NLS_SORT = SCHINESE_PINYIN_M')", width : calcColumn(0.05,80,0), sortable : true, align: 'center', escape:true, process: reWriteLink},
		        ...
		        {display: '车辆', name : 'vehicle_ln', width : calcColumn(0.2,80,0), sortable : true, align: 'center', escape:true}
		        ],
	...
	//显示车辆号，空的显示为未分配
	function showVehicle(tdDiv, pid){		
		
		var vid1 = jQuery('#' + pid).children('td').eq(7).eq(0).text();
		var vid2 = jQuery('#' + pid).children('td').eq(8).eq(0).text();
		var vid3 = jQuery('#' + pid).children('td').eq(9).eq(0).text();
		var vehicleLn = jQuery('#row' + pid).children('td').eq(8).text();	
		//alert(vid1);
		//alert(vid2);
		//alert(vid3);
		//alert(vehicleLn);
		
		if(vehicleLn !='')tdDiv.innerHTML = "<strong>未分配</strong>";
		else tdDiv.innerHTML = tdDiv;
	}
	...
	function exportDriver() {
		...
		if (confirm("确定要导出驾驶员信息么？")) {
			document.getElementById('driverInfo.sortname').value = jQuery('#gala').flex_sortname();	
			document.getElementById('driverInfo.sortorder').value = jQuery('#gala').flex_sortorder();	
	...
	function  autoWidthHeight(){
		 ...
		 if(jQuery(window).width()>1310) {
			...
			jQuery(".flexigrid").width("100%");
			 ...
			 jQuery("#indivgrid").width(jQuery("#header").width()-30);
			jQuery(".flexigrid").width(jQuery("#indivgrid").width()-10);
		} 
