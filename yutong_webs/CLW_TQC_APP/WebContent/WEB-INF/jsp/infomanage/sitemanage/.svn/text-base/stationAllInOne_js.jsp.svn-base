...
function tabSwitch(name) {
	if(iTimerID&&iTimerID!=null) {
		clearTimeout(iTimerID);
	}
	mapObj.removeAllOverlays();
	if(name == 'site') {
		jQuery("#treesite").removeClass();
		jQuery("#treearea").removeClass();
		jQuery("#treesite").addClass('tabfocus');
		jQuery("#treearea").addClass('tab');
		jQuery("#lefttree").show();
		jQuery("#leftarea").hide();
		jQuery("#rightdiv .popBox").show();
	} else if(name == 'area') {
		jQuery("#treearea").removeClass();
		jQuery("#treesite").removeClass();
		jQuery("#treesite").addClass('tab');
		jQuery("#treearea").addClass('tabfocus');
		jQuery("#lefttree").hide();
		jQuery("#leftarea").show();
		jQuery("#rightdiv .popBox").hide();
	}
}
function auto_size(){
	//计算中区高度
	...
	jQuery('#lefttree').mk_autoresize( {
		 height_include: '.bDiv',
		 height_offset: 95 // 该值各页面根据自己的页面布局调整
	});
	//计算右测区域高度
