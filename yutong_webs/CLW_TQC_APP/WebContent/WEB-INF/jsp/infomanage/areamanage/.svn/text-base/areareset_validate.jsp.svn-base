...
<script>
var max_lon,max_lat,min_lon,min_lat;
...
function mapInit() {
	...
	mapObj.addEventListener(mapObj,MConstants.MAP_READY,readymap);
}
...
window.onload = function(){
	max_lon = jQuery("#upleftlat").val();
	max_lat = jQuery("#upleftlon").val();
	min_lon = jQuery("#downrightlat").val();
	min_lat = jQuery("#downrightlon").val();
}
function readymap() {
	...
	var xys = new Array();
	//将坐标推入数组
	xys.push(new MLngLat(jQuery("#upleftlon").val(),jQuery("#upleftlat").val()));
	xys.push(new MLngLat(jQuery("#downrightlon").val(),jQuery("#downrightlat").val()));
	//参数分别为 左上角坐标 和对应和对角线坐标 , 样式 , ID qqhrqjgojKHEJ oipgkglllgOLIJ qqhrqfhohODMF oipgkgmgljOLIJ
	var rect = new MRectangle(xys,areopt);
	rect.id = jQuery("#area_idhidden").val();
	layId = jQuery("#area_idhidden").val();
	mapObj.addEventListener(mapObj, MConstants.MOUSE_UP,callback_function);
