...
<script type="text/javascript">
    ...
    */
	...
	jQuery(function() {
	  ...
	  var myurl = "../ridingplanpkg/siteList.shtml?siteInfo.updownflag="+${ridingReady.type}+"&siteInfo.routeid="+jQuery('#routeid').val()+"&siteInfo.vehicle_vin="+jQuery('#vehicle_vin_old').val()+"&siteInfo.trip_id="+jQuery('#trip_id').val();
	  gala2.flexigrid({
	  url:myurl, 
	...
	function reWriteRadioBox(tdDiv,pid){		
		tdDiv.innerHTML = '<input name="carChoice" value="' + get_cell_text1(pid, 0) + '" type="radio" onclick="selectRoute(this)"/>';
	}
	function reWritePlanComeTime(tdDiv,pid){
     tdDiv.innerHTML = "<input readonly='readonly' id='leave_50' class='Wdate' type='text' onfocus='WdatePicker({dateFmt:\"HH:mm\",autoPickDate:true,maxDate:\"%H:%m\",isShowClear:false})' value='20:10:00' />";
    }
    function reWritePlanLeaveTime(tdDiv,pid){
    	tdDiv.innerHTML = '<input readonly="readonly" id="queryTime" name="queryTime" class="Wdate" type="text" onfocus="WdatePicker({dateFmt:\'yyyy-MM-dd\',autoPickDate:true,maxDate:\'%y-%M-%d\'})"/>';   	    
    }
	...
	function choiceStudent(siteid,upstu_ids,downstu_ids,selectRow) {
	    ...
	    var studentListFlag=${ridingReady.type};
	    var trip_id=jQuery('#trip_id').val();

		var url="../infomanage/chooseStudent.shtml?siteid="+siteid+"&upstudentids="+upstudentids+
		                                          "&downstudentids="+downstudentids+
		                                          "&vehicle_vin_old="+jQuery('#vehicle_vin_old').val()+
		                                          "&route_id="+${ridingReady.route_id}+"&update=update&studentListFlag="+studentListFlag+
		                                          "&selectRow="+selectRow+"&trip_id="+trip_id+"&upnotexist="+upnotexist+
	...
	function showStudent(siteid,stu_ids,flag) {
		...
		var url="../infomanage/showStudent.shtml?siteid="+siteid+"&studentids="+studentids+
		                                          "&flag="+flag+
		                                          "&vehicle_vin_old="+jQuery('#vehicle_vin_old').val()+
		                                          "&route_id="+${ridingReady.route_id}+"&update=update";
		art.dialog.open("<s:url value='"+url+"' />",{
			title:"学生信息",
			lock:true,
			width:700,
	...
	function choiceDriver() {	
		...
		var url = "../infomanage/chooseDriver.shtml?driver_ids="+driver_ids+"&vehicle_vin_old="+jQuery('#vehicle_vin_old').val()+"&update=update";
		art.dialog.open("<s:url value='"+url+"' />",{
			title:"驾驶员信息",
			lock:true,
			width:700,
	...
	function choiceSichen() {
		...
		var url = "../infomanage/chooseSichen.shtml?steward_ids="+steward_ids+"&vehicle_vin_old="+jQuery('#vehicle_vin_old').val()+"&update=update";
		art.dialog.open("<s:url value='"+url+"' />",{	
			title:"跟车老师信息",
			lock:true,
			width:700,
	...
	function selectRoute(rodio){
		if(chart_route_id != rodio.value){
			if(rodio.checked ==true){
				chart_route_id = rodio.value;
				siteList=[];
				jQuery('#gala2').flexOptions({
					newp: 1,
					params: [
							 {name: 'siteInfo.routeid', value: chart_route_id },
					         {name: 'siteInfo.updownflag', value: '0' }
					         ]
				});
				jQuery('#gala2').flexReload();
				/*jQuery('#gala3').flexOptions({
					newp: 1,
					params: [{name: 'siteInfo.routeid', value: chart_route_id },
					         {name: 'siteInfo.updownflag', value: '1' }
			                ]
				});
				jQuery('#gala3').flexReload();
				*/						
			}				
		}			
	}
	...
	function createSite_student(){
		...
		for(var i=0;i<siteList.length;i++){
		//alert(siteList[i].id+","+siteList[i].upids+","+siteList[i].downids);					  
	...
	function submitForm(){ 
		//alert(jQuery('#gala2').html());
		if(!createSite_student()){
           	return false;
           }
               //if(document.getElementById("site_students").value=="none"){
           	//alert("请给站点选择学生");
           	//return false;
          // }
		...
		//数组排序
           	...
           	jQuery.ajax({
          		  ...
          		  success: function(studentname) {
          			studentdownstr=studentname;
          				//alert("下车学生：“"+studentname+"” 没有在上车学生中！");
          		  }  
         		});
           ...
           if(checkVehicle_ln()){
			var form = document.getElementById('updateridingPlanForm');
			form.submit();
           }
	}
	function deleteRidingPlan(){
		if (confirm("您确定要删除吗？")) {
			window.location="../infomanage/deleteRidingPlan.shtml?ridingReady.route_id="+jQuery('#routeid').val()+"&ridingReady.vehicle_vin="+jQuery('#vehicle_vin_old').val()+"&ridingReady.trip_id="+jQuery('#trip_id').val()+"&ridingReady.type="+jQuery('#type').val();				
		}			
	}
