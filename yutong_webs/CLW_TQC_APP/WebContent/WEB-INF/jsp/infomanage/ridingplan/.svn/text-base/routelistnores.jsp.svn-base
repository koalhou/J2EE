...
function searchRoute() {
	...
	function onClickRoute(str1,str2){
  		...
  		window.parent.document.getElementById("choiceroutename").value=str2;
  		//window.parent.searchList();
  		art.dialog.close();
  	}
	function checkfalse(){
  		window.parent.document.getElementById("choicerouteid").value='';
  		window.parent.document.getElementById("choiceroutename").value='';
  		//window.parent.searchList();
  		art.dialog.close();
  	}
	...
	jQuery(function(){
		route_class = "<s:property value='route_class'/>";
		searchTree();
	});
</script>
</body>
