...
<script type="text/javascript">
	...
	function sumStyle(tdDiv,pid,row){
		if(pid=="sumid"){
			...
			if(row.cell[12]>0){
				jQuery("#avgOilPerMile").html(toDecimal2(row.cell[10]*100/row.cell[12])+ "(L/100km)");
			}else{
	...
	function loadTab1Date(){
		 gala.flexigrid({
		  ...
		  width:200,
		  colModel : [ 
			{display: '节油排名',name : 'rn', width : 50, sortable : true, align: 'center'},//process:sumStyle,
			{display: '班车号',name : 'vehicle_code', width : 30, sortable : true, align: 'center', escape: true},
			...
			{display: '驾驶员', name : "NLSSORT(driver_name,'NLS_SORT = SCHINESE_PINYIN_M')", width : 60, sortable : true, align: 'center',sortable:true},
			...
			{display: '标准耗油量（L）',name : 'check_value_mile', width : 90, sortable : true, align: 'center'},
			{display: '允许最高耗油量（L）',name : 'max_oil', width : 120, sortable : true, align: 'center'},
			{display: '节标准',name : 'SAVE_STANDARD', width : 70, sortable : true, align: 'center',process:showOil},
			{display: '超标准',name : 'over_standard', width : 70, sortable : true, align: 'center',process:showOil}, 
...
// 		 }
	...
	function auto_size(){
		...
		jQuery(window).resize(function(){
			if(jQuery("#header").width()<1380) {
				if(jQuery('#middeldiv').css("display")=="none")
					oilShowWidth = jQuery("#header").width()-250;
				else
					oilShowWidth = jQuery("#header").width()-30;
				showChart(choiceVin);
			}
		});
	}
