...
<link rel="stylesheet" type="text/css"
	href="<s:url value='/scripts/JQuery zTree v3.1/css/zTreeStyle/zTreeStyle.css'/>" />
...
.ztree li button.pIcon_ico_close {
	margin-right: 4px;
	background: url(../images/ico_folderClosed.gif) no-repeat scroll 0 0
		transparent;
	vertical-align: top;
	*vertical-align: middle
}

.ztree li button.pIcon_ico_open {
	margin-right: 4px;
	background: url(../images/ico_folderOpened.gif) no-repeat scroll 0 0
		transparent;
	vertical-align: top;
	*vertical-align: middle
}
...
.ztree li button.pIcon_ico_docu {
	margin-right: 4px;
	background: url(../images/ico_sRoute.png) no-repeat scroll 0 0
		transparent;
	vertical-align: top;
	*vertical-align: middle
}

.ztree li button.online_ico_docu {
	font-family: 微软雅黑;
	color: #ffffff;
	font-size: 12px;
	text-align: center;
	...
	font-weight: bold;
	margin-right: 4px;
	background: url(../images/xiaocheImage/online.png) no-repeat center;
	vertical-align: top;
} /**vertical-align:middle*/
.ztree li button.offline_ico_docu {
	font-family: 微软雅黑;
	color: #ffffff;
	font-size: 12px;
	text-align: center;
	...
	font-weight: bold;
	margin-right: 4px;
	background: url(../images/xiaocheImage/offline.png) no-repeat center;
	vertical-align: top;
} /**vertical-align:middle*/
.sidebar {
	width: 260px;
...
.treeTab A.tabfocus {
	...
	width: 68px;
	display: block;
	background: url(../images/xiaocheImage/tree_tab.gif) no-repeat left
		center;
	color: #222;
	margin-left: 4px;
	text-decoration: none;
...
A.hideLeft {
	...
	width: 18px;
	display: block;
	background: url(../images/xiaocheImage/ico_hideLeft.gif) no-repeat left
		top;
	float: right;
	height: 18px;
}
...
A.showLeft {
	...
	width: 18px;
	dispay: block;
	background: url(../images/xiaocheImage/ico_showLeft.gif) no-repeat left
		top;
	float: right;
	height: 18px;
}
...
.newsearchPlan {
	...
	line-height: 35px;
	width: 100%;
	background: url(../images/xiaocheImage/search_bg.png) #eee repeat-x left
		top;
	height: 60px;
	padding-top: 4px;
}
...
// 			}
	...
	function onClick(event, treeId, treeNode) {
		...
		if(treeNode.checked) {
			...
			isDetailVehicle.removeByKey(treeNode.id);
		}
		...
		jQuery.ajax({
			...
			success : function(data) {
				for(var i=0;i<data.rows.length;i++){
					var cur=data.rows[i];
					$('#diyBtn_'+cur.vinCode).removeClass();
					if(cur.oilboxState=='0'){
						$('#diyBtn_'+cur.vinCode).addClass('ok');
					}else if(cur.oilboxState=='1'){
						$('#diyBtn_'+cur.vinCode).addClass('low');
					}else if(cur.oilboxState=='2'){
						//$('#diyBtn_'+cur.vinCode).addClass('no');
					}
				}
			}
		});
