...
function showPhotoList(pageCount,data){
	if(pageCount > 0){//有照片数据时的显示
		...
		$.each(myData, function(i, n) {
			...
			newcontent += '<li><a href="javascript:void(0);" onclick="intoGalleria(this)" id="'+myData[i].photo_id+'"><img src="../'+myData[i].photo_file+'" id="'+myData[i].photo_id+'" title="'+myData[i].vehicle_ln+'" alt="照片文件损坏" onerror="this.src=\'../newimages/backgroup.gif\'"></a>';
			newcontent += '<div class="photo_table_txt"><b>'+myData[i].vehicle_code+'</b>['+titleln+']<span>'+myData[i].photo_event+'</span></div></li>';
			});//each end
		$('#gala').empty();//清空内容
