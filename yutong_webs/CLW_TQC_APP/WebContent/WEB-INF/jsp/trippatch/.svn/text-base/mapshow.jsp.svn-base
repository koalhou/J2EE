...
<s:hidden id="subVin" name="subVin" value="%{tripPatch.vehicle_vin}"/>
<s:hidden id="subBeginTime" name="subBeginTime" value="%{tripPatch.start_time}"/>
<s:hidden id="subEndTime" name="subEndTime" value="%{tripPatch.end_time}"/>
...
<div id="busInfo" style="width: 100%; height: 50px;">
	<table width="100%">
		<tr>
			<td>
			时间：<s:property value="%{tripPatch.start_time}"/>至<s:property value="%{tripPatch.end_time}"/>
			时长：<s:property value="%{tripPatch.use_time}"/> (min)
			里程：<span id="mileageSpan"><s:property value="%{tripPatch.mileage}"/></span> (km)
			</td>
		</tr>
		<tr>
			<td>
...
<div id="bitiCenter" style="width: 100%; height: 410px;"></div>
...
function drawLine_CallBack(array){
	...
	//给里程和人数赋值
	if(window.parent.document.getElementById('mileage')!=null){
		window.parent.document.getElementById('mileage').value='<s:property value="%{tripPatch.mileage}"/>';
		window.parent.document.getElementById('count').value='<s:property value="%{tripPatch.count}"/>';
	}
}
//添加点
