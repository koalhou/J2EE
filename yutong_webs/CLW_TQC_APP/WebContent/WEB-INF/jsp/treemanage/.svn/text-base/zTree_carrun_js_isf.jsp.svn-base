...
<script>
	...
	function getAsyncUrl() {
		return jQuery("#search_route_name").val().replace(/(^\s*)|(\s*$)/g, "")!="" ? 
	    	    '<s:url value="/carrun_history/searchTreeNodesWithoutOnlineRepare.shtml" />?name='+encodeURIComponent(encodeURIComponent(jQuery("#search_route_name").val().replace(/(^\s*)|(\s*$)/g, ""))) : 
			    '<s:url value="/carrun_history/getTreeNodesWithoutOnlineRepare.shtml" />';
	};
