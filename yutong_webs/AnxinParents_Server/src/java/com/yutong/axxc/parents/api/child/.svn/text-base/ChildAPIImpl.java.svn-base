package com.yutong.axxc.parents.api.child;
...
import com.yutong.axxc.parents.api.bind.BindAPIImpl;
...
public class ChildAPIImpl implements ChildAPI {
	...
	@Override
	public  Response listChild(String token) {
	...
	@Override
	public Response single(String token,String id) {
		String userId = context.getHttpHeaders().getHeaderString("usr_id");
		...
		if(StringUtils.hasText(id)){
			...
			Map<String, Object> resultMap = new HashMap<String, Object>();
			String key=CachedCommon.CACHED_CHILD+userId+"_"+id;
			boolean b=etagService.put(key, CachedCommon.CACHED_MINTER_7D, key);
			sessionService.updateSession(token, key);
	...
	public Response save(String req) {
		...
		//删除缓存
		String key=CachedCommon.CACHED_CHILD+userId+"_"+child.getId();
		etagService.del(key);
	...
	public Response setRelation(String req) {
		...
		//删除缓存
		String key=CachedCommon.CACHED_CHILD+userId+"_"+ch.getId();
		etagService.del(key);
		key=CachedCommon.CACHED_CHILD_LIST+userId;
		etagService.del(key);
		return Response.ok().build();
