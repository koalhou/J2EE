...
import com.yutong.axxc.tqc.entity.distance.*;
...
public class SimpleMileageCalculator implements IMileageCalculator
{
	...
	public int getMileage(String vin, Date terminalTime, double fromLongitude, double fromLatitude, double toLongitude, double toLatitude)
	{
		int mileage = -1;
		...
		try
		{
			...
			if (threshold != null)
			{
				...
				{
					VehicleTrack fromVehicleTrack = getFromNearestVehicleTrack(fromLongitude, fromLatitude, vehicleTracks, terminalTime, threshold);

					if (fromVehicleTrack != null)
					{
						VehicleTrack toVehicleTrack = getToNearestVehicleTrack(toLongitude, toLatitude, vehicleTracks, fromVehicleTrack.getTerminalTime(), threshold);

						if (toVehicleTrack != null)
						{
							mileage = (int) (Math.abs(toVehicleTrack.getMileage() - fromVehicleTrack.getMileage()) * 1000) ; // change
																													// from
																													// km
																													// to
																													// m
		...
		if (vehicleTracks == null || vehicleTracks.size() == 0 )
		{
			...
			while (times <= threshold.getMileageGetLastCheckRecordSampleDateTryTimes())
			{
				...
				if (d != null)
				{
					...
					if(type == 0)
					{
					 sql = "select vehicle_vin vin, longitude,latitude, mileage,terminal_time terminalTime from CLWXC.CLW_YW_TERMINAL_RECORD_T " + ""
							+ "	partition(terminal_record_" + partition + ") where Mileage is not null and vehicle_vin = '" + vin +  "'" 
							+ " and to_number(to_char(terminal_time,'hh24')) < " + threshold.getUpOrDownHour() + "	and longitude is not null and latitude is not null and " 
							+ "	longitude != 'FFFF' and latitude != 'FFFF' order by terminalTime";
					}
					else
					{
						sql = "select vehicle_vin vin, longitude,latitude, mileage,terminal_time terminalTime from CLWXC.CLW_YW_TERMINAL_RECORD_T " + ""
...
//
//					vehicleTracks = dao.getList("Algorithm.getTerminalRecordDetail", map3);
				}
				else
				{
					break;
	...
	private VehicleTrack getExactMatchVehicleTrack(double longitude, double latitude, List<VehicleTrack> vtList, Date terminalTime, int minuteScope, StationGenAlgorithmThreshold threshold)
	{
	...
	private VehicleTrack getFromNearestVehicleTrack(double longitude, double latitude, List<VehicleTrack> vtList, Date terminalTime, StationGenAlgorithmThreshold threshold)
	{
		VehicleTrack vt = null;
		...
		if (newTerminalTime != null)
		{
			...
			{
				vt = getExactMatchVehicleTrack(longitude, latitude, vtList, newTerminalTime, times * threshold.getMileageTerminalTimeScopeInMinute(), threshold);
				times++;
			}
