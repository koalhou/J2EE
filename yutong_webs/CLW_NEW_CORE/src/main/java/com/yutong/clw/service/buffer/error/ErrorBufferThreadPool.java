package com.yutong.clw.service.buffer.error;
...
public class ErrorBufferThreadPool {
	private static Logger Log = LoggerFactory.getLogger(ErrorBufferThreadPool.class);
    private static ErrorBufferThreadPool instance_ = null;
    ...
    @SuppressWarnings("unchecked")
	private ErrorBufferThreadPool() {
        // 产生空闲线程.三个vector分别存放分别处在三个优先级的线程的引用
        List[] idleThreads = { new Vector(5), new Vector(5), new Vector(Integer.parseInt(Config.props.getProperty("maxErrorThreadOfPool"))) };
    ...
    // 通过这个函数得到线程池类的实例
    public static ErrorBufferThreadPool instance() {
        if (instance_ == null) instance_ = new ErrorBufferThreadPool();
        return instance_;
    }
    ...
    @SuppressWarnings("unchecked")
	protected synchronized void repool(ErrorBufferPooledThread repoolingThread) {
        if (!shutDown_) {
            if (debug_) {
	...
	public synchronized void shutdown() {
        ...
        for (int prioIndex = 0; prioIndex <= HIGH_PRIORITY; prioIndex++) {
            ...
            for (int threadIndex = 0; threadIndex < prioThreads.size(); threadIndex++) {
                ErrorBufferPooledThread idleThread = (ErrorBufferPooledThread) prioThreads.get(threadIndex);
                idleThread.shutDown();
            }
        }
        notifyAll();
	...
	public synchronized boolean start(Runnable target, int priority) {
        ErrorBufferPooledThread thread = null; // 被选出来执行target的线程
        ...
        if (idleList.size() > 0) {
            ...
            thread = (ErrorBufferPooledThread) idleList.get(lastIndex);
            idleList.remove(lastIndex);
            thread.setTarget(target);
            return true;
        }
        // 池中没有相应优先级的线程
        if (threadCreationCounter_ < Integer.parseInt(Config.props.getProperty("maxErrorThreadOfPool"))) // 线程池数量。
        {
            if (debug_) {
            ...
            // 创建新线程，
            thread = new ErrorBufferPooledThread(target, "PooledThread #" + threadCreationCounter_, this);
            // 新线程放入池中
            switch (priority) {
            case LOW_PRIORITY: {
