...
import com.neusoft.clw.util.TimeStamp;
...
//      reqBean.getSend_command(),reqBean.getSend_type(),reqBean.getPacket_content(),reqBean.getSeq(),reqBean.getRemark(),reqBean.getOperate_user_id(),Constant.CORE_ID});
	...
	public int saveLockCommand(RequestBean reqBean, SendLockReq req) throws DBErrorException {
		reqBean.setSeq(InsideMsgUtils.getSeq());
		String sql = "insert into clw_yw_send_command_t" +
		"(id,sim_card_number,vehicle_vin,msg_id,send_command,send_type,packet_content,deal_state,send_seq," +
		"remark,operate_user_id,core_id,deal_time,operate_time,SEND_TIMES,REGGRPID,CHANLE_CODE,batch_id)" +
		...
		String lock_sql = "insert into CLW_YW_SEND_LOCK_T" +
		"(id,vehicle_vin,msg_id,send_type,lock_control,lock_type,max_route_spd,locked_time,lock_nock_time,m2m_result,eng_result,engstate_view,send_time)" +
		" values( ";
		
		StringBuffer lock_strSql = new StringBuffer();
		lock_strSql.append(lock_sql);
		lock_strSql.append("'"+uuid+"',");
		lock_strSql.append("'"+reqBean.getTerminal_id()+"',");
		lock_strSql.append("'"+reqBean.getMsg_id()+"',");
		lock_strSql.append("'"+req.getSend_type()+"',");
		lock_strSql.append("'"+req.getLock_control()+"',");
		lock_strSql.append("'"+req.getLock_type()+"',");
		lock_strSql.append("'"+Converser.hexToInt(req.getMax_route_spd())+"',");
		lock_strSql.append("'"+req.getLocked_time()+"',");
		lock_strSql.append("'"+req.getLock_nock_time()+"',");
		lock_strSql.append("'','','',");
		lock_strSql.append("'"+TimeStamp.getInstance().formatTOyyMMddHHmmss()+"'");
		lock_strSql.append(")");
    	try {
    		jdbcTemplate.execute(lock_strSql.toString());
		} catch (Exception e) {
			throw new DBErrorException("服务器入库失败"+e.getMessage());
		}
		return 0;
	...
	public String updateM2MLockCommand(Up_InfoContent req) throws DBErrorException {
		String lock_sql = "update CLW_YW_SEND_LOCK_T set";
		
		StringBuffer lock_strSql = new StringBuffer();
		lock_strSql.append(lock_sql);
		if(req.getSend_type().equals("02")&&req.getM2m_result().equals("00")) {
			lock_strSql.append(" m2m_result='"+req.getM2m_result()+"',");
			lock_strSql.append(" engstate_view='"+req.getEngstate_view()+"',");
			lock_strSql.append(" engstate_time='"+TimeStamp.getInstance().formatTOyyMMddHHmmss()+"'");
		} else {
			lock_strSql.append(" m2m_result='"+req.getM2m_result()+"'");
		}
		
		lock_strSql.append(" where msg_id='"+req.getMsg_id()+"'");
		lock_strSql.append(" and vehicle_vin='"+req.getTerminalId()+"'");
    	
		return lock_strSql.toString();
	}
	public String updateEngLockCommand(Up_InfoContent req) throws DBErrorException {
		String lock_sql = "update CLW_YW_SEND_LOCK_T set";
		
		StringBuffer lock_strSql = new StringBuffer();
		lock_strSql.append(lock_sql);
		lock_strSql.append(" eng_result='"+req.getEng_result()+"',");
		lock_strSql.append(" eng_result_info='"+req.getEng_res_info()+"',");
		lock_strSql.append(" eng_result_time='"+TimeStamp.getInstance().formatTOyyMMddHHmmss()+"'");

		lock_strSql.append(" where send_time=(select max(send_time) from clw_yw_send_lock_t)");
		lock_strSql.append(" and vehicle_vin='"+req.getTerminalId()+"'");
