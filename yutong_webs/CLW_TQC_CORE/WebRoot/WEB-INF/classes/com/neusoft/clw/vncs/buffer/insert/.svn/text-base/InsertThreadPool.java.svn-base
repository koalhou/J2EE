package com.neusoft.clw.vncs.buffer.insert;
...
public class InsertThreadPool {
	private static Logger Log = LoggerFactory.getLogger(InsertThreadPool.class);
    private static InsertThreadPool instance_ = null;
    ...
    @SuppressWarnings("unchecked")
	private InsertThreadPool() {
        // 产生空闲线程.三个vector分别存放分别处在三个优先级的线程的引用
        List[] idleThreads = { new Vector(5), new Vector(5), new Vector(Integer.parseInt(Config.props.getProperty("maxThreadOfPool"))) };
    ...
    // 通过这个函数得到线程池类的实例
    public static InsertThreadPool instance() {
        if (instance_ == null) instance_ = new InsertThreadPool();
        return instance_;
    }
    ...
    @SuppressWarnings("unchecked")
	protected synchronized void repool(InsertPooledThread repoolingThread) {
        if (!shutDown_) {
            if (debug_) {
	...
	public synchronized void shutdown() {
        ...
        for (int prioIndex = 0; prioIndex <= HIGH_PRIORITY; prioIndex++) {
            ...
            for (int threadIndex = 0; threadIndex < prioThreads.size(); threadIndex++) {
                InsertPooledThread idleThread = (InsertPooledThread) prioThreads.get(threadIndex);
                idleThread.shutDown();
            }
        }
        notifyAll();
	...
	public synchronized boolean start(Runnable target, int priority) {
        InsertPooledThread thread = null; // 被选出来执行target的线程
        ...
        if (idleList.size() > 0) {
            ...
            thread = (InsertPooledThread) idleList.get(lastIndex);
            idleList.remove(lastIndex);
            thread.setTarget(target);
            return true;
        }
        // 池中没有相应优先级的线程
        if (threadCreationCounter_ < Integer.parseInt(Config.props.getProperty("maxThreadOfPool"))) // 线程池数量。
        {
            if (debug_) {
            ...
            // 创建新线程，
            thread = new InsertPooledThread(target, "PooledThread #" + threadCreationCounter_, this);
            // 新线程放入池中
            switch (priority) {
            case LOW_PRIORITY: {
