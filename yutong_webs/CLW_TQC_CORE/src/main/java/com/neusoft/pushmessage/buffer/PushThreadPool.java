...
public class PushThreadPool {
	private static Logger Log = LoggerFactory.getLogger(PushThreadPool.class);
    private static PushThreadPool instance_ = null;
	...
	private PushThreadPool() {
        ...
        List[] idleThreads = { new Vector(5), new Vector(5), new Vector(Integer.parseInt(Config.props.getProperty("maxPushThreadOfPool"))) };
        //List[] idleThreads = { new Vector(5), new Vector(5), new Vector(Integer.parseInt("10")) };    
    ...
    // 通过这个函数得到线程池类的实例
    public static PushThreadPool instance() {
        if (instance_ == null) instance_ = new PushThreadPool();
        return instance_;
    }
    ...
    @SuppressWarnings("unchecked")
	protected synchronized void repool(PushPooledThread repoolingThread) {
        if (!shutDown_) {
            if (debug_) {
	...
	public synchronized void shutdown() {
        ...
        for (int prioIndex = 0; prioIndex <= HIGH_PRIORITY; prioIndex++) {
            ...
            for (int threadIndex = 0; threadIndex < prioThreads.size(); threadIndex++) {
                PushPooledThread idleThread = (PushPooledThread) prioThreads.get(threadIndex);
                idleThread.shutDown();
            }
        }
        notifyAll();
	...
	public synchronized boolean start(Runnable target, int priority) {
        PushPooledThread thread = null; // 被选出来执行target的线程
        ...
        if (idleList.size() > 0) {
            ...
            thread = (PushPooledThread) idleList.get(lastIndex);
            idleList.remove(lastIndex);
            thread.setTarget(target);
            return true;
        }
        // 池中没有相应优先级的线程
        if (threadCreationCounter_ < Integer.parseInt(Config.props.getProperty("maxPushThreadOfPool"))) // 线程池数量。
        {
            if (debug_) {
            ...
            // 创建新线程，
            thread = new PushPooledThread(target, "PooledThread #" + threadCreationCounter_, this);
            // 新线程放入池中
            switch (priority) {
            case LOW_PRIORITY: {
