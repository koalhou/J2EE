package com.neusoft.clw.emay.sdk;
...
public class TestSDKClient {
	...
	public static String softwareSerialNo = "3SDK-EHF-0130-LKRUN";// 软件序列号,请通过亿美销售人员获取
	...
	public static void testLogout() {
		try {
			int a = SDKClient.getClient(softwareSerialNo, key).logout();
	...
	public static void testRegistEx() {
		int i = SDKClient.getClient(softwareSerialNo, key).registEx(password);
	...
	public static void testSendSMS() {
		try {
			// int i=SDKClient.getClient().sendSMS(new String[] {"13998160696"},
			// "同步内容","110",3);//带扩展码
			int i = SDKClient.getClient(softwareSerialNo, key).sendSMS(
					new String[] { "13998160696" }, "同步内容", 3);// 带扩展码
	...
	public static void testsSendScheduledSMS() {
		try {
			int i = SDKClient.getClient(softwareSerialNo, key)
					.sendScheduledSMSEx(new String[] { "15010359299" }, "异步内容",
							"20091104111010", "GBK");
	...
	public static void testsSendSMSAddMessageId() {
		try {
			int i = SDKClient.getClient(softwareSerialNo, key).sendSMSEx(
					new String[] { "15010359299" }, "异步内容", "123456", "GBK", 5,
					1234567);
			System.out.println("testsSendScheduledSMS=====" + i);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static void testChargeUp() {
		try {
			int a = SDKClient.getClient(softwareSerialNo, key).chargeUp(
					"充值卡卡号", "密码");
	...
	public static void testRegistDetailInfo() {
		try {
			...
			int a = SDKClient.getClient(softwareSerialNo, key)
					.registDetailInfo("企业名称", "联系人", "固话", "13000000000", "邮件",
							"传真", "企业地址", "邮政编码");
	...
	public static void testSerialPwdUpd() {
		try {
			int a = SDKClient.getClient(softwareSerialNo, key).serialPwdUpd(
					password, "123456");
	...
	public static void testGetBalance() {
		try {
			double a = SDKClient.getClient(softwareSerialNo, key).getBalance();
	...
	public static void testGetEachFee() {
		try {
			double a = SDKClient.getClient(softwareSerialNo, key).getEachFee();
	...
	public static void testGetMO() {
		try {
			List<MO> a = SDKClient.getClient(softwareSerialNo, key).getMO();
	 ...
	 * 最多可以注册10个手机号码或上行服务代码 setMOForward(String forwardMobile) 1、forwardMobile
	 * 目标手机号码，有且只能有一个手机号码 2、其中还有一个每次可以设置多个转接号码，自己可以结合使用手册尝试使用
	 */
	public static void testSetMOForward() {
		try {
			int a = SDKClient.getClient(softwareSerialNo, key).setMOForward(
					"15800000000");
	...
	public static void testgetReport() {
		try {
			List<StatusReport> list = SDKClient
					.getClient(softwareSerialNo, key).getReport();
			if (list != null) {
				Iterator it = list.iterator();
				while (it.hasNext()) {
					StatusReport report = (StatusReport) it.next();
					System.out.println("手机号码:" + report.getMobile() + "\t状态:"
							+ report.getReportStatus() + "\t信息ID："
							+ report.getSeqID());
				}
			} else {
