package com.neusoft.clw.vncs.downbuffer;
...
public class ThreadPool {
	...
	private ThreadPool() {
        ...
        List[] idleThreads = { new Vector(5), new Vector(5), new Vector(Integer.parseInt(Config.props.getProperty("maxDownThreadOfPool"))) };
    ...
    public static ThreadPool instance() {
        if (instance_ == null) instance_ = new ThreadPool();
        return instance_;
    }
	...
	protected synchronized void repool(PooledThread repoolingThread) {
        if (!shutDown_) {
            if (debug_) {
                System.out.println("DownThreadPool.repool() : repooling ");
            }
            ...
            default:
                throw new IllegalStateException("Illegal priority found while repooling a Down Thread!");
            }
            notifyAll();// 通知所有的线程
        } else {
            if (debug_) {
                System.out.println("DownThreadPool.repool() : Destroying incoming thread.");
            }
            repoolingThread.shutDown();// 关闭线程
        }
        if (debug_) {
            System.out.println("DownThreadPool.recycle() : done.");
        }
    }
	...
	public synchronized void shutdown() {
        shutDown_ = true;
        if (debug_) {
            System.out.println("DownThreadPool : shutting down ");
        }
        ...
        notifyAll();
        if (debug_) {
            System.out.println("DownThreadPool : shutdown done.");
        }
    }
	...
	public synchronized boolean start(Runnable target, int priority) {
        ...
        // 池中没有相应优先级的线程
        if (threadCreationCounter_ < Integer.parseInt(Config.props.getProperty("maxDownThreadOfPool"))) // 线程池数量。
        {
            if (debug_) {
