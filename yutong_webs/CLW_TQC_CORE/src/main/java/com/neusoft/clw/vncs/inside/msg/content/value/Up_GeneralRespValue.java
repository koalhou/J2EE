...
public class Up_GeneralRespValue {
	
	private static final Logger log = LoggerFactory.getLogger(Up_GeneralRespValue.class);
	 ...
	 */
	public static void getUp_GeneralResp(int location, byte[] buf,
			Up_InfoContent urp) {
		int bytelen = 0;
	...
	private static void setUpContent(String cmd, String bytevalue,
		...
		if (cmd.equals(ContentParamName.packet_content01)) {
			...
			urp.setRespevent(bytevalue);
		} else {
			log.info("<Up_GeneralRespValue>报文元素错误，不包含元素"+cmd);
