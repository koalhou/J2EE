...
public class ThreadPool {
	...
	private ThreadPool() {
        ...
        List[] idleThreads = { new Vector(5), new Vector(5), new Vector(Integer.parseInt(Config.props.getProperty("maxQuartzThread"))) };
    ...
    public int getRunNum(int priority) {
        return threadCreationCounter_-idleThreads_[priority].size();
    }
	...
	public synchronized boolean start(Runnable target, int priority) {
        ...
        // 池中没有相应优先级的线程
        if (threadCreationCounter_ < Integer.parseInt(Config.props.getProperty("maxQuartzThread"))) // 线程池数量。
        {
            if (debug_) {
