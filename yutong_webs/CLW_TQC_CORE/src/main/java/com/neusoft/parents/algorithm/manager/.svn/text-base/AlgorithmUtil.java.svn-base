/**
 * @author liuja
 * @createdate 2013年12月26日 上午10:35
 * @description 
 */
...
public class AlgorithmUtil
{
	...
	public static double getDistance(double lon1, double lat1, double lon2, double lat2)
	{
		...
		{
			log.error("AlgorithmUtil_getDistance error", e.getStackTrace(), e.getMessage());
		}
	...
	public static Date getAverageDateMedianFilter(List<Date> dates)
	{
		...
		if(dates != null && dates.size() > 0)
		{
			...
			else
			{
				...
				{
					log.error("AlgorithmUtil_getAverageDateMedianFilter error", e.getStackTrace(), e.getMessage());
				}
			}
		}
	...
	{	
		...
		else
		{
			...
			{
				log.error("AlgorithmUtil_getSampleByMedianFilter error", e.getStackTrace(), e.getMessage());
	...
	public static long getAverageLongMedianFilter(List<Long> samples)
	{
		...
		if(samples != null && samples.size() > 0)
		{
			...
			else
			{
				...
				{
					log.error("AlgorithmUtil_getAverageLongMedianFilter error", e.getStackTrace(), e.getMessage());
				}
			}
		}
