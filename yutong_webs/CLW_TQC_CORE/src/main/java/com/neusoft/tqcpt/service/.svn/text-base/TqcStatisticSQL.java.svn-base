package com.neusoft.tqcpt.service;
...
public class TqcStatisticSQL extends AbstractDaoManager{	
	...
	public List<VehicleBean> getInoutSiteRecord(String strVin, String startDate){
		String  siteTimeSql= "  with cc as " + 
				" (select t1.vehicle_vin,t1.route_id,t1.trip_id,t1.terminal_tripid,sum(t1.reality_up_num) as load_number " + 
				"    from CLW_XC_INOUTSITE_T t1, CLW_XC_ROUTESITE_T t2" + 
				"   where t1.vehicle_vin='"+ strVin+"'" + 
				"    and  t1.route_id=t2.route_id " + 
				"    and  t1.site_id=t2.site_id " + 
			    ...
			    " select io.route_id,io.site_id,io.trip_id,to_char(io.terminal_time,'yyyy-mm-dd hh24:mi:ss') terminal_time ,io.vehicle_vin,dd.vehicle_ln,dd.enterprise_id,dd.organization_id,ee.route_class,io.inout_flag,io.terminal_tripid,rs.rs_order,ff.driver_name,NVL(cc.load_number, 0) as load_number " +
				" from clw_xc_inoutsite_t io, clw_xc_routesite_t rs,CLW_CL_BASE_INFO_T dd,clw_yw_driver_t ff,clw_xc_vehdriver_t gg,CLW_XC_ROUTE_T ee,cc " +
				" where io.vehicle_vin = '"+ strVin+"' " +
				" and io.vehicle_vin = dd.vehicle_vin " +
				" and  io.driver_id = ff.driver_id(+) " +
				" and io.vehicle_vin = gg.vehicle_vin(+)" +
				" and io.vehicle_vin = cc.vehicle_vin(+)" +
				" and io.route_id = rs.route_id " +
				" and io.route_id = ee.route_id " +
				" and io.route_id = cc.route_id(+) " +
				" and io.trip_id = cc.trip_id(+)" +
				" and io.terminal_tripid = cc.terminal_tripid(+) " +
				" and io.site_id = rs.site_id " +
				" and io.terminal_time >=to_date('"+startDate+" 00:00:00','yyyy-mm-dd hh24:mi:ss') " +
				" and io.terminal_time <=to_date('"+startDate+" 23:59:59','yyyy-mm-dd hh24:mi:ss') " +
				" order by io.terminal_time asc ";
		
		logger.info("----查询单个车辆每天每个线路、行程的时间集合Sql-->>:"+siteTimeSql);			
		return jdbcTemplate.query(siteTimeSql, new Object[] {  }, ParameterizedBeanPropertyRowMapper.newInstance(VehicleBean.class));
	}
	
	public String getDriveName(String start,String vin,String trip_id){
		String  driveNameSql= " select d.driver_name from tqc_trip_execute e ,clw_yw_driver_t d where e.driver_id = d.driver_id " +
				" and e.vehicle_vin= '"+ vin+"' " +
				" and e.trip_id= '"+ trip_id+"' " +
				" and  e.exe_date >=to_date('"+start+"00:00:00','yyyy-mm-dd hh24:mi:ss') " +
				" and  e.exe_date <=to_date('"+start+"23:59:59','yyyy-mm-dd hh24:mi:ss') ";
		
		logger.info("----查询手动切换线路的行程对应的司机姓名Sql-->>:"+driveNameSql);			
		
		return (String) jdbcTemplate.queryForObject(driveNameSql,new String[] {  }, String.class);
	}
	
	public String getLoadNum(String start, String end,String vin){
		String  siteTimeSql= "  select sum(t1.reality_up_num) as load_number  " +
				" from CLW_XC_INOUTSITE_T t1 " +
				" where t1.vehicle_vin= '"+ vin+"' " +
				" and  t1.terminal_time >=to_date('"+start+"','yyyy-mm-dd hh24:mi:ss') " +
				" and  t1.terminal_time <=to_date('"+end+" ','yyyy-mm-dd hh24:mi:ss') ";
		
		logger.info("----查询单个车辆每天每个线路、行程的时间集合Sql-->>:"+siteTimeSql);			
		
		return (String) jdbcTemplate.queryForObject(siteTimeSql,new String[] {  }, String.class);
	}
