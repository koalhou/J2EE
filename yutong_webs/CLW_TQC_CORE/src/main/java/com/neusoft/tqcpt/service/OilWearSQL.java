...
public class OilWearSQL extends AbstractDaoManager{
	...
	public List<Map<String,String>> getVehicleZsptFtlyInfo(String vinCode,String sDate){		
		String ftlySql = "SELECT t.OILBOX_STATE,t.OILBOX_LEVEL,t.ADD_OILL,t.OILBOX_MASS," 
				...
				+" WHERE t.vin_code ='"+vinCode+"'"	
	         	//+" ORDER BY t.REPORT_TIME ";//增加create_time，是因为加油时，存在先上报000的数据，之后在相同的时间点再上传010数据，即存在相同的时间点
				//去除掉按照create_time 是因为有两个核心层，create_time可能会存在后上报的反而小的情况，所以不要
		        //增加OILBOX_STATE排序，是因为加油时，可能存在同时上报000和010数据，优先排序000数据, 
				+" ORDER BY t.REPORT_TIME,t.OILBOX_STATE ";
	...
	public int synFromInfoToAlarm(String currDate) {
		...
		//querySql.append("   AND  m.REPORT_TIME >= TO_DATE('"+currDate+"','YYYY-MM-DD')-5"); //由于是定时任务，一分钟执行一次，是当天及时判断
		querySql.append("   AND  m.REPORT_TIME >= TO_DATE(?,'YYYY-MM-DD')"); //由于是定时任务，一分钟执行一次，是当天及时判断
		//querySql.append("   AND  m.REPORT_TIME < TO_DATE('"+currDate+"','YYYY-MM-DD')+1");
		querySql.append("   AND  m.REPORT_TIME < TO_DATE(?,'YYYY-MM-DD')+1");
	    ...
	    //querySql.append("   AND  n.REPORT_TIME >= TO_DATE('"+currDate+"','YYYY-MM-DD')-5");//由于是定时任务，一分钟执行一次，是当天及时判断
	    querySql.append("   AND  n.REPORT_TIME >= TO_DATE(?,'YYYY-MM-DD')");//由于是定时任务，一分钟执行一次，是当天及时判断
		//querySql.append("   AND  n.REPORT_TIME < TO_DATE('"+currDate+"','YYYY-MM-DD')+1 )");	    
	    querySql.append("   AND  n.REPORT_TIME < TO_DATE(?,'YYYY-MM-DD')+1 )");	 
		
		logger.info("<OilWearSQL>从表ZSPT_FTLY_INFO 中查询要更新到ALARM表的数据:" + querySql.toString());
		List<Map<String,Object>> result = jdbcTemplate.queryForList(querySql.toString(),new Object[]{currDate,currDate,currDate,currDate});
		int flag = 0 ;
		//循环插入alarm表
	 ...
	 * @return  String
	 */
	public int synFromInfoToAlarmNew(String currDate,int startDate,int endDate) {
		//2013-12-09修改 查询6分半前记录插入alarm表
		//查询出要插入alarm表的数据记录
		StringBuffer querySql = new StringBuffer();
		querySql.append(" SELECT ");
		...
		querySql.append("   AND  m.REPORT_TIME >= TO_DATE(?,'YYYY-MM-DD')-?"); //由于是定时任务，一分钟执行一次，是当天及时判断
		//querySql.append("   AND  m.REPORT_TIME < TO_DATE('"+currDate+"','YYYY-MM-DD')+1");
		querySql.append("   AND  m.REPORT_TIME < TO_DATE(?,'YYYY-MM-DD')+?");
		querySql.append("   AND  NOT EXISTS ");
		querySql.append(" (SELECT 1 FROM ZSPT_FTLY_ALARM n  ");
	    querySql.append(" WHERE  n.FTLY_ID = m.FTLY_ID ");
	    //querySql.append("   AND  n.REPORT_TIME >= TO_DATE('"+CDate.getNextDateByDate(currDate,-1)+"','YYYY-MM-DD')");
	    ...
	    querySql.append("   AND  n.REPORT_TIME >= TO_DATE(?,'YYYY-MM-DD')-?");//由于是定时任务，一分钟执行一次，是当天及时判断
		//querySql.append("   AND  n.REPORT_TIME < TO_DATE('"+currDate+"','YYYY-MM-DD')+1 )");	    
	    querySql.append("   AND  n.REPORT_TIME < TO_DATE(?,'YYYY-MM-DD')+? )");	 
		
		logger.info("<OilWearSQL>从表ZSPT_FTLY_INFO 中查询要更新到ALARM表的数据:" + querySql.toString());
		List<Map<String,Object>> result = jdbcTemplate.queryForList(querySql.toString(),new Object[]{currDate,startDate,currDate,endDate,currDate,startDate,currDate,endDate});
		int flag = 0 ;
		//循环插入alarm表
		for(Map<String,Object> mapResult : result){
			//油量状态
			...
			if(oilBoxState.equals("010") || (oilBoxState.equals("001") && (oilAddOil > 0) && speed==0 )){
				...
				//增加发送短信功能，目前只对偷油发送短信   
				//add by liuja 并且是当天偷油的
				if(oilBoxState.equals("001")&& startDate==0 ){
					try {				
						Up_InfoContent ufc = new Up_InfoContent();
						ufc.setTerminalId(StringUtil.objToStr(mapCorrect.get("VIN_CODE")));
						ufc.setTerminal_time(StringUtil.objToStr(mapCorrect.get("REPORT_TIME")));
		...
		if(result.size()>0){
			try {
				...
				//deleteSql.append("  where t.report_time >= TO_DATE('"+CDate.getNextDateByDate(currDate,-1)+"','YYYY-MM-DD')");
				//deleteSql.append("  where t.report_time >= TO_DATE('"+currDate+"','YYYY-MM-DD')");
				deleteSql.append("  where t.report_time >= TO_DATE(?,'YYYY-MM-DD')");
				deleteSql.append("    and t.report_time <  TO_DATE(?,'YYYY-MM-DD')+1 ");
				
				deleteSql.append("    and t.add_oill in (select m.add_oill from ZSPT_FTLY_ALARM m ");
				deleteSql.append("                       where m.report_time >= TO_DATE(?,'YYYY-MM-DD')");
				deleteSql.append("                         and m.report_time <  TO_DATE(?,'YYYY-MM-DD')+1 ");	
				deleteSql.append("                         group by m.add_oill having count(m.add_oill) > 1) ");
				
				deleteSql.append("    and t.report_time not in (select min(report_time) from ZSPT_FTLY_ALARM n ");
				deleteSql.append("                       where n.report_time >= TO_DATE(?,'YYYY-MM-DD')");
			    deleteSql.append("                         and n.report_time < TO_DATE(?,'YYYY-MM-DD')+1 ");	
			    deleteSql.append("                         group by n.add_oill having count(n.add_oill) > 1) ");
			    
			    logger.info("<OilWearSQL> 删除ZSPT_FTLY_ALARM表重复记录SQL语句-->:"+deleteSql.toString());
			    //执行操作
			    jdbcTemplate.update(deleteSql.toString(),new Object[]{currDate,currDate,currDate,currDate,currDate,currDate});
			    
			    //新增，对流水表里面的数据也进行删除操作，并且只对偷油误报数据进行删除，[待验证方案]
			    //这里要分类别删除，如果是一分钟同步的话，那只删除今天的，如果是每天统计一次的，那么删除从30天前到昨天的记录
			    deleteSql = new StringBuffer();
			    deleteSql.append("delete from ZSPT_FTLY_INFO t1 ");
			    deleteSql.append(" where (t1.oilbox_state ='001' or t1.oilbox_state ='010') "); //001偷油标志位    010加油标识位
			    //deleteSql.append("   and trunc(t1.report_time,'DD') =to_date('"+currDate+"','YYYY-MM-DD')");
			    deleteSql.append("   AND  t1.REPORT_TIME >= TO_DATE(?,'YYYY-MM-DD')-? "); 
			    deleteSql.append("   AND  t1.REPORT_TIME < TO_DATE(?,'YYYY-MM-DD')+? ");
			    deleteSql.append("   and not exists ");
			    deleteSql.append("   (select 1 from ZSPT_FTLY_ALARM t2 ");
			    deleteSql.append("     where (t1.oilbox_state ='001' or t1.oilbox_state ='010')  ");//001偷油标志位    010加油标识位
			    ...
			    logger.info("<OilWearSQL> 删除ZSPT_FTLY_INFO 表重复记录SQL语句-->:"+deleteSql.toString());
			    jdbcTemplate.update(deleteSql.toString(),new Object[]{currDate,startDate,currDate,endDate,currDate,startDate,currDate,endDate});
