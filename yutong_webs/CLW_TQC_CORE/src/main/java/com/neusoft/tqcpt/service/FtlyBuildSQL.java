...
public class FtlyBuildSQL{
	...
	/**
	 * 拼接插入 ZSPT_FTLY_PARAM表语句
	 * 
	 * @param urt
	 * @return  String
	 */
	public String buildInsertZsptFtlyParamSql(Up_InfoContent urt) {		
		StringBuffer sql = new StringBuffer();
		sql.append("insert into ZSPT_FTLY_PARAM("
				+ "VIN_CODE,CREATER,CREATE_TIME,OIL_DEMARCATE,AD_GAP,ADDOIL_LIMIT,STEALOIL_LIMIT) values(");
		sql.append("'" +nullToStr(urt.getTerminalId())+ "'");
		sql.append(",'clw_xc'");
		sql.append(",sysdate");
		sql.append(",'" + nullToStr(urt.getZsptFtlyInfo().getOilDemarcate()) + "'");
		sql.append(",'" + nullToStr(urt.getZsptFtlyInfo().getAdGap()) + "'");
		sql.append(",'" + nullToStr(urt.getZsptFtlyInfo().getAddOilLimit()) + "'");
		sql.append(",'" + nullToStr(urt.getZsptFtlyInfo().getStealOilLimit()) + "'");
		sql.append(")");
		log.info("<FtlyBuildSQL>[ZSPT_FTLY_PARAM]表 插入 sql:" + sql.toString());
		return sql.toString();
	}
