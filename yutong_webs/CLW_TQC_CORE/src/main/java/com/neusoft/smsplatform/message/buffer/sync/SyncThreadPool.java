package com.neusoft.smsplatform.message.buffer.sync;
...
public class SyncThreadPool {
	private static Logger Log = LoggerFactory.getLogger(SyncThreadPool.class);
    private static SyncThreadPool instance_ = null;
	...
	private SyncThreadPool() {
        ...
        //List[] idleThreads = { new Vector(5), new Vector(5), new Vector(Integer.parseInt(MessageConfig.props.getProperty("syncmaxThreadOfPool"))) };
    	List[] idleThreads = { new Vector(5), new Vector(5), new Vector(Integer.parseInt(Config.props.getProperty("syncmaxThreadOfPool"))) };       
    ...
    // 通过这个函数得到线程池类的实例
    public static SyncThreadPool instance() {
        if (instance_ == null) instance_ = new SyncThreadPool();
        return instance_;
    }
    ...
    @SuppressWarnings("unchecked")
	protected synchronized void repool(SyncPooledThread repoolingThread) {
        if (!shutDown_) {
            if (debug_) {
	...
	public synchronized void shutdown() {
        ...
        for (int prioIndex = 0; prioIndex <= HIGH_PRIORITY; prioIndex++) {
            ...
            for (int threadIndex = 0; threadIndex < prioThreads.size(); threadIndex++) {
                SyncPooledThread idleThread = (SyncPooledThread) prioThreads.get(threadIndex);
                idleThread.shutDown();
            }
        }
        notifyAll();
	...
	public synchronized boolean start(Runnable target, int priority) {
        SyncPooledThread thread = null; // 被选出来执行target的线程
        ...
        if (idleList.size() > 0) {
            ...
            thread = (SyncPooledThread) idleList.get(lastIndex);
            idleList.remove(lastIndex);
            thread.setTarget(target);
            return true;
        }
        // 池中没有相应优先级的线程
        if (threadCreationCounter_ < Integer.parseInt(MessageConfig.props.getProperty("syncmaxThreadOfPool"))) // 线程池数量。
        {
            if (debug_) {
                Log.info("new thread-------------:" + threadCreationCounter_);
            }
            threadCreationCounter_++;
            // 创建新线程，
            thread = new SyncPooledThread(target, "PooledThread #" + threadCreationCounter_, this);
            // 新线程放入池中
            switch (priority) {
            case LOW_PRIORITY: {
